...
}(u), ca = v.source, da = v.output, ea = v.version, fa = v.help, ha = v.ignore, ia = v.noSourceMaps, ja = v.extensions || "js,jsx", ka = v.jsx, la = v.preact;
const {basename:ma, dirname:w, join:x, relative:na} = path;
const {appendFileSync:oa, chmodSync:pa, createReadStream:y, createWriteStream:z, lstat:A, lstatSync:qa, mkdir:ra, readdir:sa, readlink:ta, symlink:ua, writeFileSync:va} = fs;
const wa = (a, b = 0, c = !1) => {
  if (0 === b && !c) {
    return a;
  }
...
}, xa = (a, b = !1) => wa(a, 2 + (b ? 1 : 0)), ya = a => {
  ({callee:{caller:a}} = a);
  return a;
};
const {homedir:C} = os;
const za = /\s+at.*(?:\(|\s)(.*)\)?/, Aa = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, Ba = C(), D = a => {
  ...
  }).filter(f => f.trim()).map(f => b ? f.replace(za, (g, h) => g.replace(h, h.replace(Ba, "~"))) : f).join("\n");
};
function Ca(a, b, c = !1) {
  return function(d) {
    var e = ya(arguments), {stack:f} = Error();
...
async function F(a, b, c) {
  ...
  return await new Promise((f, g) => {
    ...
    Array.isArray(b) ? (b.forEach((l, m) => {
      Da(e, m);
    }), k = [...b, h]) : 1 < Array.from(arguments).length && (Da(e, 0), k = [b, h]);
    a(...k);
  });
}
;async function G(a) {
...
;async function Ea(a, b) {
  b = b.map(async c => {
    const d = x(a, c);
    return {lstat:await F(A, d), path:d, relativePath:c};
  });
...
async function I(a) {
  ...
  if (!(await F(A, a)).isDirectory()) {
    throw a = Error("Path is not a directory"), a.code = "ENOTDIR", a;
  }
  var b = await F(sa, a);
  b = await Ea(a, b);
  a = b.filter(Fa);
  b = b.filter(Ga).reduce((c, d) => {
...
;const Ha = async(a, b) => {
  ...
  await G(x(b, "path.file"));
  const {content:c} = await I(a), d = Object.keys(c).map(async e => {
    const {type:f} = c[e], g = x(a, e);
    e = x(b, e);
    "Directory" == f ? await Ja(g, e) : "File" == f ? await Ha(g, e) : "SymbolicLink" == f && await Ia(g, e);
  });
  await Promise.all(d);
}, Ka = async(a, b) => {
  const c = await F(A, a), d = ma(a);
  b = x(b, d);
  c.isDirectory() ? await Ja(a, b) : c.isSymbolicLink() ? await Ia(a, b) : (await G(b), await Ha(a, b));
};
var La = stream;
...
class Pa extends Na {
  constructor(a) {
    ...
    const {U:f = E(!0), proxyError:g} = a || {}, h = (k, l) => f(l);
    super(b);
    this.a = [];
...
const J = async a => {
  var b = void 0 === b ? {} : b;
  ({b:a} = new Pa(Object.assign({}, {rs:a}, b, {U:E(!0)})));
  return await a;
};
async function Qa(a) {
  a = y(a);
...
;async function Sa(a) {
  ...
  b && !d && (d = y(b));
  "-" == c ? d.pipe(process.stdout) : c ? await Ta(c, d, b) : e instanceof Na && (d.pipe(e), await new Promise((f, g) => {
    e.on("error", g);
    e.on("finish", f);
  }));
}
...
;const ab = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), bb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, cb = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
    const {getReplacement:f = bb, getRegex:g = ab} = b || {}, h = g(d);
...
const gb = (a, b = []) => {
  ...
  c = !1;
  let g = 1, h;
  K(k, [{re:new RegExp(`[\\s\\S](?:<\\s*${b}(\\s+|>)|/\\s*${b}\\s*>)`, "g"), replacement(l, m, n, p) {
    if (c) {
      return l;
    }
    m = !m && l.endsWith(">");
...
const jb = (a, b = {}) => {
  ...
  a = `${f}${q}${a}`;
  return jb(a, b);
}, U = (a, b = !1, c = null) => a ? ib(a).reduce((d, e) => {
  if (e instanceof R) {
    const {l:h = "", content:k, tagName:l} = e, {D:m, B:n} = P(h);
    e = U(k, b, c);
    e = Q(l, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {m:{length:h}, l:k = "", content:l, tagName:m} = S(g), {D:n, B:p} = P(k);
    g = U(l, b, c);
    g = Q(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + h);
...
const kb = (a, b = {}) => {
  const {e:c, S:d, T:e, v:f, X:g, Y:h} = cb({S:/^ *export\s+default\s+{[\s\S]+?}/mg, e:/^ *export\s+(?:default\s+)?/mg, T:/^ *export\s+{[^}]+}\s+from\s+(['"])(?:.+?)\1/mg, v:/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?\s+from\s+['"].+['"]/gm, X:/^ *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+.+?\s+from\s+['"].+['"]/gm, Y:/^ *import\s+['"].+['"]/gm}, {getReplacement(k, l) {
    return `/*%%_RESTREAM_${k.toUpperCase()}_REPLACEMENT_${l}_%%*/`;
  }, getRegex(k) {
    return new RegExp(`/\\*%%_RESTREAM_${k.toUpperCase()}_REPLACEMENT_(\\d+)_%%\\*/`, "g");
  }});
  a = K(a, [M(e), M(d), M(c), M(f), M(g), M(h)]);
  b = jb(a, b);
  return K(b, [L(e), L(d), L(c), L(f), L(g), L(h)]);
};
const lb = (a, b) => {
  a = qa(a);
  ({mode:a} = a);
...
const mb = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".split("");
function V(a) {
  var b = "";
  a = 0 > a ? (-a << 1) + 1 : a << 1;
  do {
...
const pb = /^[A-Za-z0-9+\-.]+:/;
function Y(a) {
  return "/" == a[0] ? "/" == a[1] ? "scheme-relative" : "path-absolute" : pb.test(a) ? "absolute" : "path-relative";
}
function qb(a, b) {
...
const rb = function(a) {
  return b => {
    ...
    a(b);
    b = b.toString();
    return "absolute" === c ? b : "scheme-relative" === c ? b.slice(5) : "path-absolute" === c ? b.slice(11) : qb(d, b);
  };
}(() => {
});
function sb(a, b) {
  a: {
    if (Y(a) !== Y(b)) {
      ...
      c = c.protocol !== a.protocol || c.user !== a.user || c.password !== a.password || c.hostname !== a.hostname || c.port !== a.port ? null : qb(a, c);
    }
  }
  return "string" == typeof c ? c : rb(b);
}
;class tb {
  constructor() {
    this.a = [];
...
class xb {
  ...
  toJSON() {
    ...
    this.a && (b.sourcesContent = wb(this, a, this.b));
    return b;
  }
  toString() {
...
;function zb(a, b) {
  function c() {
    return b.filter(yb).reduce((d, {re:e, replacement:f}) => {
      if (this.c) {
        return d;
      }
      if ("string" == typeof f) {
...
;const Ab = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), Bb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, Eb = () => {
  var a = {comments:Cb, inlineComments:Db, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm};
  return Object.keys(a).reduce((b, c) => {
    {
      var d = a[c];
      const {getReplacement:e = Bb, getRegex:f = Ab} = {}, g = f(c);
...
const Hb = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:h} = Eb(), k = [b, c, d, e, f, g, h], [l, m, n, p, q, t, B] = k.map(Gb), [O, Ub, Vb, Wb, Xb, Yb, Zb] = k.map($b => Fb($b));
  return {rules:[q, l, m, n, B, t, p, ...a, Wb, Yb, Zb, Vb, Ub, O, Xb], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:h}};
};
const Ib = ({file:a, F:b, aa:c, sourceRoot:d}) => {
  const e = new xb({file:a, sourceRoot:d});
  b.replace(Cb, (f, g) => {
    if ("\n" == b[g + f.length]) {
      return "\n".repeat(f.split("\n").length - 1);
    }
...
function Jb({source:a, $:b, name:c, destination:d, file:e, F:f}) {
  a = na(b, a);
  e = Ib({file:e, F:f, aa:a});
  c = `${c}.map`;
  oa(d, `\n//# sourceMappingURL=${c}`);
  b = x(b, c);
  va(b, e);
...
class Nb extends Ma {
  constructor(a, b) {
    super(b);
    this.rules = (Array.isArray(a) ? a : [a]).filter(Kb);
    this.c = !1;
    this.a = b;
  }
  async replace(a, b) {
    const c = new Nb(this.rules, this.a);
    b && Object.assign(c, b);
    a = await Mb(c, a);
    c.c && (this.c = !0);
...
;const Ob = a => `if (${a} && ${a}.__esModule) ${a} = ${a}.default`, Pb = (a, b, c, d = null) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (e, f, g) => `${f}=${g}`)}${d ? d : "r" + `equire(${b}${c}${b})`}`, Qb = (a, b = {}) => {
  if (!b.import) {
    return a;
  }
...
}, Rb = /(\s+from\s+)(?:%%_RESTREAM_STRINGS_REPLACEMENT_(\d+)_%%|%%_RESTREAM_LITERALS_REPLACEMENT_(\d+)_%%)/, Sb = (a = {import:{}}) => {
  try {
...
const Tb = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), ac = (a, b, c, d, e) => {
  if (!a) {
    return {};
  }
  b = b ? {d:`${b} = ${"r" + `equire(${c}${d}${c})`}`, C:Ob(b)} : void 0;
  const {d:f, C:g} = b;
...
const cc = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Rb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  f = Qb(a, this.config);
  a = /^[./]/.test(f) && !Sb(this.config);
  const {t:l, C:m} = ac(b, c, h, f, a);
  b = bc(d, e, h, f, c);
  return `${[l, b, ...a ? [] : [m]].filter(n => n).join("; ")};`;
}}, {re:new RegExp(`${/( *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+(.+?))/.source}${Rb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  const [, h, k] = /(["'`])(.+?)\1/.exec(g ? this.markers.literals.map[g] : this.markers.strings.map[f]);
  a = Pb(e, h, k);
  ({length:b} = b.split("\n"));
  b = "\n".repeat(b - 1);
  e = /^[./]/.test(k) && !Sb(this.config);
  return `${c ? [`${b}${e ? "const" : "let"} ${d} = ${c}${a}`, ...e ? [] : [Ob(c)]].join("; ") : `${b}const ${d}${a}`};`;
}}];
...
const oc = async a => {
  ...
  [, a] = await Promise.all([Sa(Object.assign({}, {source:b}, d ? {writable:d} : {destination:c}, {readable:e})), J(a), new Promise((f, g) => e.on("finish", f).on("error", g))]);
  return a;
};
const pc = Wa("alamode"), rc = async({input:a, A:b, name:c, i:d, g:e, h:f, extensions:g}) => {
  const h = x(b, c);
  e.includes(h) || e.some(k => h.startsWith(`${k}/`)) || (e = "-" == d, a = x(a, h), b = e ? null : x(d, b), e = e ? "-" : x(b, c), pc(h), await G(e), qc(h, g) ? (g = await oc({source:a, destination:e}), "-" != d && (lb(a, e), f || Jb({destination:e, file:h, name:c, $:b, source:a, F:g}))) : await Sa({source:a, destination:e}));
}, tc = async({input:a, i:b, A:c = ".", g:d = [], h:e, extensions:f, o:g, w:h}) => {
  if ("-" == b) {
    throw Error("Output to stdout is only for files.");
  }
  const k = x(a, c), l = x(b, c), {content:m} = await I(k);
  await Object.keys(m).reduce(async(n, p) => {
    await n;
    const q = x(k, p);
    n = x(l, p);
    const {type:t} = m[p];
    "File" == t ? g && /jsx$/.test(p) ? (p = await sc(q, h), n = n.replace(/jsx$/, "js"), await G(n), await Ra(n, p)) : g ? await Ka(q, l) : await rc({input:a, A:c, name:p, i:b, g:d, h:e, extensions:f, o:g}) : "Directory" == t && (p = x(c, p), await tc({input:a, i:b, g:d, A:p, h:e, extensions:f, o:g}));
  }, {});
}, sc = async(a, b) => {
  var c = await Qa(a);
  c = await kb(c, {quoteProps:"dom", warn(d) {
    console.warn(Va(d, "yellow"));
    console.warn(Va(" in %s", "grey"), a);
  }});
  return b ? `import { h } from 'preact'
${c}` : c;
}, qc = (a, b) => b.some(c => a.endsWith(c)), uc = async({input:a, i:b = "-", g:c = [], h:d, extensions:e, o:f, w:g}) => {
  if (!a) {
  ...
  if (h.isDirectory()) {
    ...
    await tc({input:a, i:b, g:c, h:d, extensions:e, o:f, w:g});
  } else {
    h.isFile() && (h = ma(a), f && /jsx$/.test(h) ? (c = "-" == b ? "-" : x(b, h), a = await sc(a, g), "-" == c ? console.log(a) : (g = c.replace(/jsx$/, "js"), await G(g), await Ra(g, a))) : await rc({input:w(a), A:".", name:h, i:b, g:c, h:d, extensions:e, w:g}));
  }
  "-" != b && process.stdout.write(`Transpiled code saved to ${b}\n`);
};
function vc() {
  const {usage:a = {}, description:b, line:c, example:d} = {usage:wc, description:"A tool to transpile JavaScript packages using regular expressions.\nSupports import/export and JSX transpilation.\nhttps://artdecocode.com/alamode/", line:"alamode source [-o destination] [-i dir,file] [-s] [-jp]", example:"alamode src -o build"};
  var e = Object.keys(a);
...
;if (fa) {
  ...
  ea && (console.log("v%s", require("../../package.json").version), process.exit());
}
(async() => {
  try {
    const a = ha ? ha.split(",") : [], b = ja.split(",");
    await uc({input:ca, i:da, h:ia, g:a, extensions:b, o:ka, w:la});
  } catch (a) {
