...
boolean:!0, short:"s"}, extensions:{description:"Files of what extensions to transpile.", default:"js,jsx", short:"e"}, jsx:{description:"Enable JSX mode: only update JSX syntax to use hyperscript.\nDoes not transpile `import/export` statements.", boolean:!0, short:"j"}, preact:{description:'When transpiling JSX, automatically insert at the top\n`import { h } from "preact"`.', boolean:!0, short:"p"}}, u = function(a, b) {
  ...
  return Object.keys(a).reduce((e, f) => {
    ...
    return void 0 === l ? Object.assign({}, {j:e}, g) : Object.assign({}, {j:e}, g, {[f]:l});
  }, {j:b});
...
async function D(a, b, c) {
  ...
  return await new Promise((f, g) => {
    ...
    Array.isArray(b) ? (b.forEach((k, m) => {
      Fa(e, m);
    }), l = [...b, h]) : 1 < Array.from(arguments).length && (Fa(e, 0), l = [b, h]);
    a(...l);
  });
}
;async function E(a) {
...
class Ra extends Pa {
  constructor(a) {
    ...
    const {U:f = C(!0), proxyError:g} = a || {}, h = (l, k) => f(k);
    super(b);
    this.a = [];
    ...
    return b.filter(bb).reduce((d, {re:e, replacement:f}) => {
      ...
      if ("string" == typeof f) {
        ...
        return d.replace(e, (h, ...l) => {
          g = Error();
          try {
            return this.c ? h : f.call(this, h, ...l);
          } catch (k) {
            I(g, k);
          }
        });
...
;const gb = a => {
  ...
  return g || b.length ? `{${f.reduce((h, l) => {
    const k = a[l], m = c || -1 != l.indexOf("-") ? `'${l}'` : l, {before:n = "", I:p = "", H:q = ""} = d[l] || {};
    return [...h, `${n}${m}${p}:${q}${k}`];
  }, b).join(",")}${e}}` : "{}";
}, ib = (a = "") => {
  [a] = a;
...
const jb = (a, b = []) => {
  ...
  c = !1;
  let g = 1, h;
  J(l, [{re:new RegExp(`[\\s\\S](?:<\\s*${b}(\\s+|>)|/\\s*${b}\\s*>)`, "g"), replacement(k, m, n, p) {
    if (c) {
      return k;
    }
    m = !m && k.endsWith(">");
  ...
  return new R({m:l, l:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class R {
  constructor(a) {
    this.m = a.m;
...
const mb = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {l:g = "", content:h, tagName:l, m:{length:k}} = S(f);
  f = U(h, c, d);
  ...
  if (e instanceof R) {
    const {l:h = "", content:l, tagName:k} = e, {D:m, B:n} = O(h);
    e = U(l, b, c);
    e = Q(k, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {m:{length:h}, l = "", content:k, tagName:m} = S(g), {D:n, B:p} = O(l);
    g = U(k, b, c);
    g = Q(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + h);
...
const nb = (a, b = {}) => {
  const {e:c, S:d, T:e, v:f, X:g, Y:h} = K({S:/^ *export\s+default\s+{[\s\S]+?}/mg, e:/^ *export\s+(?:default\s+)?/mg, T:/^ *export\s+{[^}]+}\s+from\s+(['"])(?:.+?)\1/mg, v:/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?\s+from\s+['"].+['"]/gm, X:/^ *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+.+?\s+from\s+['"].+['"]/gm, Y:/^ *import\s+['"].+['"]/gm}, {getReplacement(l, k) {
  ...
  toJSON() {
    ...
    var b = this.f.a.slice();
    {
      ...
      for (let q = 0, t = d.length; q < t; q++) {
        ...
        c = e.generatedColumn;
        null != e.source && (p = this.u.indexOf(e.source), n += V(p - k), k = p, n += V(e.originalLine - 1 - h), h = e.originalLine - 1, n += V(e.originalColumn - g), g = e.originalColumn, null != e.name && (e = this.f.indexOf(e.name), n += V(e - l), l = e));
        m += n;
      }
      c = m;
    }
...
const Db = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:h} = K({comments:Bb, inlineComments:Cb, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), l = [b, c, d, e, f, g, h], [k, m, n, p, q, t, B] = l.map(M), [P, Ob, Pb, Qb, Rb, Sb, Tb] = l.map(Ub => L(Ub));
  return {rules:[q, k, m, n, B, t, p, ...a, Qb, Sb, Tb, Pb, Ob, P, Rb], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:h}};
};
const Eb = ({file:a, F:b, aa:c, sourceRoot:d}) => {
  ...
  b.replace(Bb, (f, g) => {
    ...
    const h = g + 1;
    f.replace(/(?:(?:\s+)|(?:[$_\w\d]+)|.)/g, (l, k) => {
      if (0 != k || !/^\s+$/.test(l)) {
        l = {line:h, column:k};
        {
          let {L:m, N:n = null, source:p = null, name:q = null} = {L:l, source:c, N:l};
          if (!m) {
...
const Vb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Jb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  const [, h, l] = /(["'`])(.+?)\1/.exec(g ? this.markers.literals.map[g] : this.markers.strings.map[f]);
  a = l.replace(this.markers.regexes.regExp, (n, p) => this.markers.regexes.map[p]);
  f = Ib(a, this.config);
  a = /^[./]/.test(f) && !Kb(this.config);
  const {t:k, C:m} = Mb(b, c, h, f, a);
  b = Nb(d, e, h, f, c);
...
function cc(a, b, c, d, e) {
  ...
  e = Yb(e).reduce((g, h) => {
    ...
    h = l.trim();
    g[(k ? k.trim() : null) || h] = "default" == h ? f : `${f}.${h}`;
    return g;
  }, {});
  this.emit("export", e);
  return b;
...
const hc = Ya("alamode"), jc = async({input:a, A:b, name:c, i:d, g:e, h:f, extensions:g}) => {
  const h = w(b, c);
  e.includes(h) || e.some(l => h.startsWith(`${l}/`)) || (e = "-" == d, a = w(a, h), b = e ? null : w(d, b), e = e ? "-" : w(b, c), hc(h), await E(e), ic(h, g) ? (g = await gc({source:a, destination:e}), "-" != d && (ob(a, e), f || Fb({destination:e, file:h, name:c, $:b, source:a, F:g}))) : await Ua({source:a, destination:e}));
}, lc = async({input:a, i:b, A:c = ".", g:d = [], h:e, extensions:f, o:g, w:h}) => {
  if ("-" == b) {
    throw Error("Output to stdout is only for files.");
  }
  const l = w(a, c), k = w(b, c), {content:m} = await G(l);
  await Object.keys(m).reduce(async(n, p) => {
    await n;
    const q = w(l, p);
    n = w(k, p);
    const {type:t} = m[p];
    "File" == t ? g && /jsx$/.test(p) ? (p = await kc(q, h), n = n.replace(/jsx$/, "js"), await E(n), await Ta(n, p)) : g ? await Ma(q, k) : await jc({input:a, A:c, name:p, i:b, g:d, h:e, extensions:f, o:g}) : "Directory" == t && (p = w(c, p), await lc({input:a, i:b, g:d, A:p, h:e, extensions:f, o:g}));
  }, {});
}, kc = async(a, b) => {
  var c = await Sa(a);
...
${c}` : c;
}, ic = (a, b) => b.some(c => a.endsWith(c)), mc = async({input:a, i:b = "-", g:c = [], h:d, extensions:e, o:f, w:g}) => {
  if (!a) {
  ...
  if (h.isDirectory()) {
    ...
    h.isFile() && (h = na(a), f && /jsx$/.test(h) ? (c = "-" == b ? "-" : w(b, h), a = await kc(a, g), "-" == c ? console.log(a) : (g = c.replace(/jsx$/, "js"), await E(g), await Ta(g, a))) : await jc({input:v(a), A:".", name:h, i:b, g:c, h:d, extensions:e, w:g}));
  }
...
function nc() {
  ...
  const l = [b, `  ${c || ""}`].filter(k => k ? k.trim() : k).join("\n\n");
  e = `${l ? `${l}\n` : ""}
${e.join("\n")}
`;
  return d ? `${e}
...
(async() => {
  try {
    ...
    await mc({input:da, i:ea, h:ja, g:a, extensions:b, o:la, w:ma});
  } catch (a) {
