...
const Va = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), eb = (a, b, c, d, e, f, g) => {
  const {t:k, m:h} = Wa(c, d, e, f, g);
  a = Xa(a, b, e, f, d);
...
const hb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Ka.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  const l = Ja(a, this.config);
  this.F || (this.F = {});
  if (this.async) {
    return gb(l, this.config, this.file, this.F).then(m => eb(d, e, b, c, k, l, m));
  }
  a = fb(l, this.config);
  return eb(d, e, b, c, k, l, a);
...
const Ib = a => {
  const b = Fb(), {rules:c, markers:d} = Gb(), e = new Hb(b, d);
  return c.reduce((f, g) => {
...
module.exports = a => {
  a = void 0 === a ? {} : a;
  const b = Fb(), {pragma:c = 'const { h } = require("preact");', noWarning:d = !1, matcher:e, ignoreNodeModules:f} = Object.assign({}, a, b);
