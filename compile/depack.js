...
class ta extends ja {
  constructor(a) {
    const {binary:b = !1, rs:c = null, ...d} = a || {}, {M:e = K(!0), proxyError:f} = a || {}, g = (h, k) => e(k);
    super(d);
    this.a = [];
...
}, N = (a, b = {}, c = [], d = [], e = !1, f = null, g = {}, h = "") => {
  const k = xa(a), l = k ? a : `'${a}'`;
...
const ya = (a, b = []) => {
  ...
  return new O({g:k, f:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class O {
  constructor(a) {
    this.g = a.g;
...
const Ba = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {f:g = "", content:h, tagName:k, g:{length:l}} = P(f);
  f = R(h, c, d);
  const {o:m, l:n, h:p} = M(g.replace(/^ */, ""));
  d = N(k, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  c = a.slice(0, e);
  a = a.slice(e + l);
  e = l - d.length;
  0 < e && (d = `${" ".repeat(e)}${d}`);
  ...
  if (e instanceof O) {
    const {f:h = "", content:k, tagName:l} = e, {o:m, l:n} = M(h);
    e = R(k, b, c);
    e = N(l, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {g:{length:h}, f:k = "", content:l, tagName:m} = P(g), {o:n, l:p} = M(k);
    g = R(l, b, c);
    g = N(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + h);
...
const Ga = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:h} = C({comments:Da, inlineComments:Ea, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), k = [b, c, d, e, f, g, h], [l, m, n, p, q, r, t] = k.map(E), [H, Xa, Ya, Za, $a, ab, bb] = k.map(cb => D(cb));
  return {rules:[q, l, m, n, t, r, p, Fa, ...a, Za, ab, bb, Ya, Xa, H, $a], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:h}};
}, Fa = {re:/./, replacement(a) {
...
const hb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Ka.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  const l = (f = Va(this.file, k, this.config)) || Ja(a, this.config);
...
const Gb = () => {
  const a = [...hb, ...rb], {rules:b, markers:c} = Ga(a);
