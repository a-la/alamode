...
function z(a, b = {}) {
  ...
  f.forEach(m => {
    ...
    h[m] = y._extensions[m] = function(p, q) {
      let r;
      !g && ha(q, f, e, d) && (r = p._compile, p._compile = function(t) {
        p._compile = r;
        t = a(t, q);
        if ("string" != typeof t) {
...
;const ma = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), na = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, C = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
    const {getReplacement:f = na, getRegex:g = ma} = b || {}, h = g(d);
...
const oa = os.homedir;
const F = /\s+at.*(?:\(|\s)(.*)\)?/, pa = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, qa = oa(), G = a => {
  ...
  }).filter(f => f.trim()).map(f => b ? f.replace(F, (g, h) => g.replace(h, h.replace(qa, "~"))) : f).join("\n");
};
const ra = (a, b = 0, c = !1) => {
  if (0 === b && !c) {
    return a;
...
function ua(a, b, c = !1) {
  return function(d) {
    ...
    const g = ra(f, 2, !0), h = (f = d instanceof Error) ? d.message : d;
...
class wa extends ka {
  constructor(a) {
    const {binary:b = !1, rs:c = null, ...d} = a || {}, {M:e = I(!0), proxyError:f} = a || {}, g = (h, k) => e(k);
    super(d);
    this.a = [];
...
}, L = (a, b = {}, c = [], d = [], e = !1, f = null, g = {}, h = "") => {
  const k = Aa(a), l = k ? a : `'${a}'`;
...
const Ba = (a, b = []) => {
  ...
  var f = e.replace(/<\s*[^\s/>]+/, "");
  if (/\/\s*>$/.test(f)) {
    return a = f.replace(/\/\s*>$/, ""), f = "", new M({g:e.replace(d.regExp, "=>"), f:a.replace(d.regExp, "=>"), content:"", tagName:b});
  }
  a = f.replace(/>$/, "");
  f = c + 1;
  c = !1;
  ...
  return new M({g:k, f:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class M {
  constructor(a) {
    this.g = a.g;
...
;const O = a => {
  ...
  return b.length ? Ca(a, b) : [O(a)];
}, Ca = (a, b) => {
  let c = 0;
  b = b.reduce((d, {from:e, to:f, N:g, O:h}) => {
    (e = a.slice(c, e)) && d.push(O(e));
    c = f;
...
const Ea = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {f:g = "", content:h, tagName:k, g:{length:l}} = N(f);
  f = P(h, c, d);
  const {o:m, l:n, h:p} = K(g.replace(/^ */, ""));
  d = L(k, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  c = a.slice(0, e);
  a = a.slice(e + l);
  e = l - d.length;
  0 < e && (d = `${" ".repeat(e)}${d}`);
  a = `${c}${d}${a}`;
  return Ea(a, b);
}, P = (a, b = !1, c = null) => a ? Da(a).reduce((d, e) => {
  if (e instanceof M) {
    const {f:h = "", content:k, tagName:l} = e, {o:m, l:n} = K(h);
    e = P(k, b, c);
    e = L(l, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {g:{length:h}, f:k = "", content:l, tagName:m} = N(g), {o:n, l:p} = K(k);
    g = P(l, b, c);
    g = L(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + h);
...
const Ga = /\/\*(?:[\s\S]+?)\*\//g, Ha = /\/\/(.+)/gm;
const Ja = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:h} = C({comments:Ga, inlineComments:Ha, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), k = [b, c, d, e, f, g, h], [l, m, n, p, q, r, t] = k.map(E), [H, $a, ab, bb, cb, db, eb] = k.map(fb => D(fb));
  return {rules:[q, l, m, n, t, r, p, Ia, ...a, bb, db, eb, ab, $a, H, cb], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:h}};
}, Ia = {re:/./, replacement(a) {
  this.stopProcessing && this.brake();
  return a;
}};
const Ka = a => `if (${a} && ${a}.__esModule) ${a} = ${a}.default`, La = (a, b, c, d = null) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (e, f, g) => `${f}=${g}`)}${d ? d : "r" + `equire(${b}${c}${b})`}`, Q = (a, b = {}) => {
  if (!b.import) {
    return a;
  }
...
}, Ma = /(\s+from\s+)(?:%%_RESTREAM_STRINGS_REPLACEMENT_(\d+)_%%|%%_RESTREAM_LITERALS_REPLACEMENT_(\d+)_%%)/, Na = (a = {import:{}}) => {
  try {
...
;const Qa = fs.createReadStream, Ra = fs.lstat;
const R = async a => {
  try {
    return await Pa(Ra, a);
  } catch (b) {
    return null;
  }
};
...
const Va = async(a, b, c = {}) => {
  ...
  let k;
  try {
    ({path:k} = await Ta(a)), a = k;
  } catch (l) {
  }
  return {entry:a, version:e, packageName:f, main:!d && g, entryExists:!!k, ...h};
};
const Wa = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), Xa = (a, b, c = {}) => {
  if (!c.import) {
    return null;
  }
  ({import:{stdlib:c}} = c);
  ...
  if (/^[./]/.test(a) || ba.includes(a) || Oa(a, b)) {
    return !0;
  }
}, ib = async(a, b, c, d) => {
  if (hb(a, b)) {
  ...
  if (c) {
    try {
      const {alamode:e} = await Va(u(c), a, {fields:["alamode"]});
      d[a] = !!e;
      return e;
    } catch (e) {
      return !1;
  ...
  b = b ? {d:`${b} = ${"r" + `equire(${c}${d}${c})`}`, m:Ka(b)} : void 0;
  const {d:f, m:g} = b;
...
const jb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Ma.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  const l = (f = Xa(this.file, k, this.config)) || Q(a, this.config);
  f && (d ? b && (d = d.replace(/{/, `{ ${c},`), d = b.replace(/\S/g, " ") + d, c = b = void 0) : (d = b.replace(/(\S+)/, "{ $1 }"), c = b = void 0));
  this.F || (this.F = {});
  if (this.async) {
    return ib(l, this.config, this.file, this.F).then(m => gb(d, e, b, c, h, l, m));
  }
  a = hb(l, this.config);
  return gb(d, e, b, c, h, l, a);
  ...
  b = Q(f, this.config);
  return a + `${e}${b}${e});`;
}}, {re:new RegExp(`${/( *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+(.+?))/.source}${Ma.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  const [, h, k] = /(["'`])(.+?)\1/.exec(g ? this.markers.literals.map[g] : this.markers.strings.map[f]);
  a = Q(k, this.config);
  e = La(e, h, a);
  ({length:b} = b.split("\n"));
  b = "\n".repeat(b - 1);
  a = /^[./]/.test(a) && !Na(this.config);
  return `${c ? [`${b}${a ? "const" : "let"} ${d} = ${c}${e}`, ...a ? [] : [Ka(c)]].join("; ") : `${b}const ${d}${e}`};`;
}}];
...
const qb = (a, b, c) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (d, e, f) => `${e}=${f}`)}${"r" + `equire(${b}${c}${b});`}`;
...
;const Gb = ({file:a, T:b, U:c, sourceRoot:d}) => {
  const e = new Fb({file:a, sourceRoot:d});
  b.replace(Ga, (f, g) => {
    if ("\n" == b[g + f.length]) {
      return "\n".repeat(f.split("\n").length - 1);
    }
    ...
    g = " ".repeat(g[f].length);
    return `${"\n".repeat(f)}${g}`;
  }).replace(Ha, f => " ".repeat(f.length)).split("\n").forEach((f, g) => {
    const h = g + 1;
...
const Ib = () => {
  const a = [...jb, ...tb], {rules:b, markers:c} = Ja(a);
...
const Kb = (a, b) => {
  const c = Hb(), {rules:d, markers:e} = Ib(), f = new Jb(c, e, b);
...
*/
module.exports = (a = {}) => {
  const b = Hb(), {pragma:c = 'const { h } = require("preact");', noWarning:d = !1, matcher:e, ignoreNodeModules:f} = {...a, ...b};
  ...
  global.ALAMODE_JS = z(Mb, {exts:[".js"], matcher:e, ignoreNodeModules:f});
