...
boolean:!0, short:"p"}, "preact-externs":{description:'Same as `preact`, but imports from `\uff20externs/preact`\n`import { h } from "\uff20externs/preact"`.', boolean:!0, short:"E"}, debug:{description:"Will make \u00c0LaMode stop after replacing markers.", boolean:!0, short:"d"}}, r = function(a = {}, b = process.argv) {
  [, , ...b] = b;
  const c = ba(b);
  b = b.slice(c.length);
  let d = !c.length;
...
}(ca), ea = r.source, fa = r.output, ha = r.version, ia = r.help, ja = r.ignore, ka = r.noSourceMaps, la = r.extensions || "js,jsx", ma = r.jsx, na = r.module, oa = r.preact, pa = r["preact-externs"], qa = r.debug;
const {basename:ra, dirname:t, join:v, relative:w, resolve:sa} = path;
const {appendFileSync:ta, chmodSync:ua, createReadStream:x, createWriteStream:y, existsSync:va, lstat:z, lstatSync:wa, mkdir:xa, readFileSync:ya, readdir:za, readlink:Aa, symlink:Ba, writeFileSync:A} = fs;
const Ca = (a, b = 0, c = !1) => {
  if (0 === b && !c) {
    return a;
  }
...
const Ga = /\s+at.*(?:\(|\s)(.*)\)?/, Ha = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, Ia = Fa(), B = a => {
  ...
  }).filter(f => f.trim()).map(f => b ? f.replace(Ga, (g, l) => g.replace(l, l.replace(Ia, "~"))) : f).join("\n");
};
function Ja(a, b, c = !1) {
  return function(d) {
    var e = Ea(arguments), {stack:f} = Error();
    const g = Ca(f, 2, !0), l = (f = d instanceof Error) ? d.message : d;
...
async function D(a, b, c) {
  ...
  return await new Promise((f, g) => {
    ...
    Array.isArray(b) ? (b.forEach((h, m) => {
      Ka(e, m);
    }), k = [...b, l]) : 1 < Array.from(arguments).length && (Ka(e, 0), k = [b, l]);
    a(...k);
  });
}
;async function F(a) {
...
;const Oa = async(a, b) => {
  ...
  const c = await D(z, a), d = ra(a);
  b = v(b, d);
  c.isDirectory() ? await Qa(a, b) : c.isSymbolicLink() ? await Pa(a, b) : (await F(b), await Oa(a, b));
};
var Sa = stream;
...
;const pb = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), qb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, R = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
    const {getReplacement:f = qb, getRegex:g = pb} = b || {}, l = g(d);
...
class tb extends Ta {
  ...
  async replace(a, b) {
    const c = new tb(this.rules, this.a);
    b && Object.assign(c, b);
    a = await rb(c, a);
    c.c && this.brake();
...
;const ub = /\/\*(?:[\s\S]+?)\*\//g, vb = /\/\/(.+)/gm;
const xb = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:l} = R({comments:ub, inlineComments:vb, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), k = [b, c, d, e, f, g, l], [h, m, n, p, q, u, E] = k.map(T), [X, Ub, Vb, Wb, Xb, Yb, Zb] = k.map($b => S($b));
  return {rules:[q, h, m, n, E, u, p, wb, ...a, Wb, Yb, Zb, Vb, Ub, X, Xb], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:l}};
}, wb = {re:/./, replacement(a) {
  this.stopProcessing && this.brake();
  return a;
}};
const yb = ({file:a, v:b, O:c, sourceRoot:d}) => {
  ...
  b.replace(ub, (f, g) => {
    ...
    g = " ".repeat(g[f].length);
    return `${"\n".repeat(f)}${g}`;
  }).replace(vb, f => " ".repeat(f.length)).split("\n").forEach((f, g) => {
    const l = g + 1;
...
function zb({source:a, ba:b, name:c, destination:d, file:e, v:f}) {
  a = w(b, a);
  e = yb({file:e, v:f, O:a});
  c = `${c}.map`;
  ta(d, `\n//# sourceMappingURL=${c}`);
...
;const Ab = a => `if (${a} && ${a}.__esModule) ${a} = ${a}.default`, Bb = (a, b, c, d = null) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (e, f, g) => `${f}=${g}`)}${d ? d : "r" + `equire(${b}${c}${b})`}`, Cb = (a, b = {}) => {
  if (!b.import) {
    return a;
  }
...
}, Db = /(\s+from\s+)(?:%%_RESTREAM_STRINGS_REPLACEMENT_(\d+)_%%|%%_RESTREAM_LITERALS_REPLACEMENT_(\d+)_%%)/, Eb = (a = {import:{}}) => {
  try {
...
const Kb = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), Lb = (a, b, c = {}) => {
  ...
  if (/^[./]/.test(a) || Jb.includes(a) || Fb(a, b)) {
    return !0;
  }
}, Qb = async(a, b, c, d) => {
  if (Pb(a, b)) {
  ...
  b = b ? {d:`${b} = ${"r" + `equire(${c}${d}${c})`}`, A:Ab(b)} : void 0;
  const {d:f, A:g} = b;
...
const Rb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Db.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  const h = (f = Lb(this.file, k, this.config)) || Cb(a, this.config);
  f && (d ? b && (d = d.replace(/{/, `{ ${c},`), d = b.replace(/\S/g, " ") + d, c = b = void 0) : (d = b.replace(/(\S+)/, "{ $1 }"), c = b = void 0));
  this.L || (this.L = {});
  if (this.async) {
    return Qb(h, this.config, this.file, this.L).then(m => Ob(d, e, b, c, l, h, m));
  }
  a = Pb(h, this.config);
  return Ob(d, e, b, c, l, h, a);
}}, {re:new RegExp(`${/( *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+(.+?))/.source}${Db.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  const [, l, k] = /(["'`])(.+?)\1/.exec(g ? this.markers.literals.map[g] : this.markers.strings.map[f]);
  a = Bb(e, l, k);
  ({length:b} = b.split("\n"));
  b = "\n".repeat(b - 1);
  e = /^[./]/.test(k) && !Eb(this.config);
  return `${c ? [`${b}${e ? "const" : "let"} ${d} = ${c}${a}`, ...e ? [] : [Ab(c)]].join("; ") : `${b}const ${d}${a}`};`;
}}];
...
const ac = a => a.split(/,\s*/).filter(b => b), bc = a => a.reduce((b, c) => {
...
const jc = async({source:a, destination:b, writable:c, debug:d, noSourceMaps:e}) => {
  ...
  d = await I(a);
  f.end(d);
  await Promise.all([Za({source:a, ...c ? {writable:c} : {destination:b}, readable:f}), new Promise((g, l) => f.on("finish", g).on("error", l))]);
  return d;
};
/*
...
}, sc = (a, b = {}, c = [], d = [], e = !1, f = null, g = {}, l = "") => {
  const k = rc(a), h = k ? a : `'${a}'`;
...
const tc = (a, b = []) => {
  ...
  var f = e.replace(/<\s*[^\s/>]+/, "");
  if (/\/\s*>$/.test(f)) {
    return a = f.replace(/\/\s*>$/, ""), f = "", new uc({h:e.replace(d.regExp, "=>"), g:a.replace(d.regExp, "=>"), content:"", tagName:b});
  }
  a = f.replace(/>$/, "");
  f = c + 1;
  c = !1;
  ...
  return new uc({h:k, g:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class uc {
  constructor(a) {
    this.h = a.h;
...
;const wc = a => {
  ...
  return b.length ? xc(a, b) : [wc(a)];
}, xc = (a, b) => {
  let c = 0;
  b = b.reduce((d, {from:e, to:f, X:g, Y:l}) => {
    (e = a.slice(c, e)) && d.push(wc(e));
    c = f;
...
const Ac = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {g = "", content:l, tagName:k, h:{length:h}} = vc(f);
  f = zc(l, c, d);
  const {B:m, w:n, m:p} = pc(g.replace(/^ */, ""));
  var q = sc(k, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  f = a.slice(0, e);
  a = a.slice(e + h);
  e = h - q.length;
  0 < e && (q = `${" ".repeat(e)}${q}`);
  f = `${f}${q}${a}`;
  return Ac(f, b);
}, zc = (a, b = !1, c = null) => a ? yc(a).reduce((d, e) => {
  if (e instanceof uc) {
    const {g:l = "", content:k, tagName:h} = e, {B:m, w:n} = pc(l);
    e = zc(k, b, c);
    e = sc(h, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {h:{length:l}, g:k = "", content:h, tagName:m} = vc(g), {B:n, w:p} = pc(k);
    g = zc(h, b, c);
    g = sc(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + l);
...
const Dc = async(a, b, c, d) => {
  var e = await I(a);
  e = await Bc(e, {quoteProps:"dom", warn(f) {
  ...
  e = e.replace(/^import (['"])(.+?\.css)\1/gm, (f, g, l) => {
    try {
      ...
      va(k) || A(k, `export default ${Cc.toString()}`);
      const h = v(t(a), l);
      l = `${l}.js`;
      const m = v(c, l);
      let n = w(t(m), k);
...
const Ec = ab("alamode"), Fc = (a, b) => a.includes(b) || a.some(c => b.startsWith(`${c}/`)), Z = async a => {
  ...
  const p = await jc({source:m, destination:a, debug:l, noSourceMaps:f});
  if ("-" != e) {
    var q = wa(m);
    ({mode:q} = q);
    ua(a, q);
    ...
    zb({destination:a, file:h, name:d, ba:n, source:m, v:p});
  } else {
    f || l || (h = yb({file:h, v:p, O:w(n || "", m)}), h = "/" + `/# sourceMappingURL=data:application/json;charset=utf-8;base64,${Buffer.from(h).toString("base64")}`, console.log("\n\n%s", h));
  }
  return a;
}, Ic = async a => {
  ...
  await Object.keys(k).reduce(async(h, m) => {
    ...
    h = v(g, m);
    var {type:n} = k[m];
    "File" == n ? Fc(a.u, v(d, m)) || (e && /jsx$/.test(m) ? (a.j && (n = m.replace(/jsx$/, "js"), h = await Z({...a, name:m, ca:n}), m = n), n = v(l, m), await Hc(h, f, c, n, {l:d, name:m, u:a.u, j:a.j})) : e && a.j ? await Z({...a, name:m}) : e ? await Ra(h, l) : await Z({...a, name:m})) : "Directory" == n && (m = v(d, m), await Ic({...a, l:m}));
  }, {});
}, Hc = async(a, b, c, d, {j:e, l:f, name:g, u:l} = {}) => {
  if (l && (f = v(f, g), Fc(l, f))) {
    return;
  }
  a = await Dc(a, b, c, e);
  ...
  await F(b);
  await Ya(b, a);
}, Gc = (a, b) => b.some(c => a.endsWith(c)), Jc = async a => {
  ...
  if (f.isDirectory()) {
    ...
    if (f.isFile()) {
      if (f = ra(b), d && /jsx$/.test(f)) {
        if (a = "-" == c ? "-" : v(c, f), f = await Hc(b, e, c, a), "-" == a) {
          return console.log(f);
        }
      } else {
        await Z({...a, input:t(b), l:"./", name:f});
      }
    }
  }
...
(async() => {
  try {
    ...
    await Jc({input:ea, C:fa, noSourceMaps:ka, u:a, extensions:b, P:c, M:ma, debug:qa, j:na});
  } catch (a) {
