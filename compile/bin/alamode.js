...
boolean:!0, short:"p"}, "preact-externs":{description:'Same as `preact`, but imports from `\uff20externs/preact`\n`import { h } from "\uff20externs/preact"`.', boolean:!0, short:"E"}, debug:{description:"Will make \u00c0LaMode stop after replacing markers.", boolean:!0, short:"d"}, require:{description:"Renames `require` calls into imports, and `module.exports` assignments to exports.\nGreat for refactoring older code.", boolean:!0, short:"r"}}, r = function(a = {}, b = process.argv) {
  ...
  b = b.slice(c.length);
  let d = !c.length;
  return Object.keys(a).reduce(({j:e, ...f}, g) => {
    if (0 == e.length && d) {
      return {j:e, ...f};
    }
    const k = a[g];
    ...
    return void 0 === l ? {j:e, ...f} : {j:e, ...f, [g]:l};
  }, {j:b});
...
;async function La(a, b) {
  b = b.map(async c => {
    const d = v(a, c);
    return {lstat:await F(z, d), path:d, relativePath:c};
  });
...
;const Oa = async(a, b) => {
  ...
  const {content:c} = await I(a), d = Object.keys(c).map(async e => {
    const {type:f} = c[e], g = v(a, e);
    e = v(b, e);
    "Directory" == f ? await Qa(g, e) : "File" == f ? await Oa(g, e) : "SymbolicLink" == f && await Pa(g, e);
  });
  await Promise.all(d);
}, Ra = async(a, b) => {
  const c = await F(z, a), d = sa(a);
  b = v(b, d);
  c.isDirectory() ? await Qa(a, b) : c.isSymbolicLink() ? await Pa(a, b) : (await G(b), await Oa(a, b));
};
var Sa = stream;
...
class Wa extends Ua {
  constructor(a) {
    const {binary:b = !1, rs:c = null, ...d} = a || {}, {W:e = D(!0), proxyError:f} = a || {}, g = (k, l) => e(l);
    super(d);
    this.a = [];
...
async function Za(a) {
  ...
  b && !d && (d = x(b));
  "-" == c ? d.pipe(process.stdout) : c ? await $a(c, d, b) : e instanceof Ua && (d.pipe(e), await new Promise((f, g) => {
    e.on("error", g);
    e.on("finish", f);
  }));
}
...
const cb = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".split("");
function M(a) {
  var b = "";
  a = 0 > a ? (-a << 1) + 1 : a << 1;
  do {
...
const fb = /^[A-Za-z0-9+\-.]+:/;
function P(a) {
  return "/" == a[0] ? "/" == a[1] ? "scheme-relative" : "path-absolute" : fb.test(a) ? "absolute" : "path-relative";
}
function gb(a, b) {
...
const hb = function(a) {
  return b => {
    ...
    a(b);
    b = b.toString();
    return "absolute" === c ? b : "scheme-relative" === c ? b.slice(5) : "path-absolute" === c ? b.slice(11) : gb(d, b);
  };
}(() => {
});
function ib(a, b) {
  a: {
    if (P(a) !== P(b)) {
      ...
      c = c.protocol !== a.protocol || c.user !== a.user || c.password !== a.password || c.hostname !== a.hostname || c.port !== a.port ? null : gb(a, c);
    }
  }
  return "string" == typeof c ? c : hb(b);
}
;class jb {
  constructor() {
    this.a = [];
  ...
  toJSON() {
    ...
    var b = this.f.a.slice();
    {
      ...
      for (let q = 0, u = d.length; q < u; q++) {
        ...
        c = e.generatedColumn;
        null != e.source && (p = this.s.indexOf(e.source), n += M(p - h), h = p, n += M(e.originalLine - 1 - k), k = e.originalLine - 1, n += M(e.originalColumn - g), g = e.originalColumn, null != e.name && (e = this.f.indexOf(e.name), n += M(e - l), l = e));
        m += n;
      }
      c = m;
    }
    ...
    this.a && (b.sourcesContent = mb(this, a, this.b));
    return b;
  }
  toString() {
...
function R(a, b) {
  function c() {
    return b.filter(ob).reduce((d, {re:e, replacement:f}) => {
      if (this.c) {
        return d;
      }
      if ("string" == typeof f) {
...
;const pb = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), qb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, S = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
...
class V extends Ta {
  constructor(a, b) {
    super(b);
    this.rules = (Array.isArray(a) ? a : [a]).filter(ob);
    this.c = !1;
    this.a = b;
  }
  async replace(a, b) {
...
const wb = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:k} = S({comments:tb, inlineComments:ub, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), l = [b, c, d, e, f, g, k], [h, m, n, p, q, u, E] = l.map(U), [Z, Xb, Yb, Zb, $b, ac, bc] = l.map(cc => T(cc));
  return {rules:[q, h, m, n, E, u, p, vb, ...a, Zb, ac, bc, Yb, Xb, Z, $b], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:k}};
}, vb = {re:/./, replacement(a) {
  this.stopProcessing && this.brake();
  return a;
}};
const xb = ({file:a, v:b, O:c, sourceRoot:d}) => {
  const e = new nb({file:a, sourceRoot:d});
  b.replace(tb, (f, g) => {
    if ("\n" == b[g + f.length]) {
      return "\n".repeat(f.split("\n").length - 1);
    }
    ...
    f.replace(/(?:(?:\s+)|(?:[$_\w\d]+)|.)/g, (l, h) => {
      if (0 != h || !/^\s+$/.test(l)) {
        l = {line:k, column:h};
        {
          let {J:m, N:n = null, source:p = null, name:q = null} = {J:l, source:c, N:l};
          if (!m) {
...
const Gb = async a => {
  ...
  if (!b) {
    ...
    if (b.isDirectory()) {
      b = !1;
      let e;
      a.endsWith("/") || (e = c = await Fb(a), b = !0);
      if (!e) {
        c = await Fb(v(a, "index"));
        if (!c) {
...
const Ib = async(a, b, c = {}) => {
  ...
  let l;
  try {
    ({path:l} = await Gb(a)), a = l;
  } catch (h) {
  }
  return {entry:a, version:e, packageName:f, main:!d && g, entryExists:!!l, ...k};
};
const {builtinModules:Jb} = _module;
const Kb = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), Lb = (a, b, c = {}) => {
  if (!c.import) {
    return null;
  }
  ({import:{stdlib:c}} = c);
  ...
  if (/^[./]/.test(a) || Jb.includes(a) || Eb(a, b)) {
    return !0;
  }
}, Qb = async(a, b, c, d) => {
  if (Pb(a, b)) {
  ...
  if (c) {
    try {
      const {alamode:e} = await Ib(t(c), a, {fields:["alamode"]});
      d[a] = !!e;
      return e;
    } catch (e) {
      return !1;
...
const Rb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Cb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
...
const Ub = a => a.split(/,\s*/).filter(b => b), Vb = a => a.reduce((b, c) => {
  ...
  ({length:a} = a.split("\n"));
  return "\n".repeat(a - 1);
}, W = (a, b = !0) => b ? dc(a) : a.split("\n").map(({length:c}, d, {length:e}) => d == e - 1 ? " ".repeat(c) : "").join("\n");
const ec = (a, b, c) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (d, e, f) => `${e}=${f}`)}${"r" + `equire(${b}${c}${b});`}`;
...
const hc = [{re:/[\s\S]*/, replacement(a) {
  this.exports = {};
  this.on("export", b => {
    this.exports = {...this.exports, ...b};
  });
  return a;
}}, {re:new RegExp(`^( *export\\s+?)( *${/(?:let|const|var|class|function\s*\*?|async +function)/.source}\\s+((?:${Tb.source}\\s*,?\\s*)+))`, "gm"), replacement:function(a, b, c, d) {
  d.split(/,\s*/).forEach(e => {
  ...
  return fc.call(this, b, k, d, g, c);
}}, {re:/^( *export\s+{([^}]+?)} *)(\n?)/gm, replacement:function(a, b, c, d) {
  a = Ub(c);
  a = Vb(a);
  this.emit("export", a);
  return this.noSourceMaps ? "" : `${W(b)}${d ? d : ""}`;
}}, {re:new RegExp(`^( *export\\s+default\\s+?)( *${/(?:class|function\s*\*?|async +function)/.source}\\s+(${Tb.source}))`, "m"), replacement:function(a, b, c, d) {
  a = d.trim();
  Wb(a);
...
const jc = async({source:a, destination:b, writable:c, debug:d, noSourceMaps:e}) => {
  const f = new ic(a);
  e && (f.noSourceMaps = e);
  d && (f.stopProcessing = !0);
  d = await J(a);
  f.end(d);
  await Promise.all([Za({source:a, ...c ? {writable:c} : {destination:b}, readable:f}), new Promise((g, k) => f.on("finish", g).on("error", k))]);
  return d;
};
/*
...
*/
const kc = {black:30, red:31, green:32, yellow:33, blue:34, magenta:35, cyan:36, white:37, grey:90};
function X(a, b) {
  return (b = kc[b]) ? `\x1b[${b}m${a}\x1b[0m` : a;
}
;const {Script:lc} = vm;
const mc = (a, b) => {
  const [c, , d] = a.split("\n");
...
const oc = a => {
  ...
  a.replace(/(\s*)(\S+)(\s*)=(\s*)(["'])([\s\S]+?)\5/g, (d, e, f, g, k, l, h, m) => {
    c[f] = {before:e, G:g, F:k};
    b.push({u:m, name:f, R:`${l}${h}${l}`});
    return "%".repeat(d.length);
  }).replace(/(\s*)([^\s%]+)/g, (d, e, f, g) => {
    c[f] = {before:e};
    b.push({u:g, name:f, R:"true"});
  });
  return {D:[...b.reduce((d, {u:e, name:f, R:g}) => {
    d[e] = [f, g];
    return d;
  ...
  return g || b.length ? `{${f.reduce((k, l) => {
    const h = a[l], m = c || -1 != l.indexOf("-") ? `'${l}'` : l, {before:n = "", G:p = "", F:q = ""} = d[l] || {};
    return [...k, `${n}${m}${p}:${q}${h}`];
  }, b).join(",")}${e}}` : "{}";
}, sc = (a = "") => {
  [a] = a;
...
}, tc = (a, b = {}, c = [], d = [], e = !1, f = null, g = {}, k = "") => {
  const l = sc(a), h = l ? a : `'${a}'`;
...
const uc = (a, b = []) => {
  ...
  var f = e.replace(/<\s*[^\s/>]+/, "");
  if (/\/\s*>$/.test(f)) {
    return a = f.replace(/\/\s*>$/, ""), f = "", new vc({i:e.replace(d.regExp, "=>"), h:a.replace(d.regExp, "=>"), content:"", tagName:b});
  }
  a = f.replace(/>$/, "");
  f = c + 1;
  c = !1;
  ...
  return new vc({i:l, h:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class vc {
  constructor(a) {
    this.i = a.i;
...
;const xc = a => {
  ...
  return b.length ? yc(a, b) : [xc(a)];
}, yc = (a, b) => {
  let c = 0;
  b = b.reduce((d, {from:e, to:f, X:g, Y:k}) => {
    (e = a.slice(c, e)) && d.push(xc(e));
    c = f;
...
const Bc = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {h:g = "", content:k, tagName:l, i:{length:h}} = wc(f);
  f = Ac(k, c, d);
  const {B:m, w:n, o:p} = qc(g.replace(/^ */, ""));
  var q = tc(l, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  f = a.slice(0, e);
  a = a.slice(e + h);
  e = h - q.length;
  0 < e && (q = `${" ".repeat(e)}${q}`);
  f = `${f}${q}${a}`;
  return Bc(f, b);
}, Ac = (a, b = !1, c = null) => a ? zc(a).reduce((d, e) => {
  if (e instanceof vc) {
    const {h:k = "", content:l, tagName:h} = e, {B:m, w:n} = qc(k);
    e = Ac(l, b, c);
    e = tc(h, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {i:{length:k}, h:l = "", content:h, tagName:m} = wc(g), {B:n, w:p} = qc(l);
    g = Ac(h, b, c);
    g = tc(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + k);
...
const Cc = (a, b = {}) => {
  const {e:c, U:d, V:e, u:f, Z:g, $:k} = S({U:/^ *export\s+default\s+{[\s\S]+?}/mg, e:/^ *export\s+(?:default\s+)?/mg, V:/^ *export\s+{[^}]+}\s+from\s+(['"])(?:.+?)\1/mg, u:/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?\s+from\s+['"].+['"]/gm, Z:/^ *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+.+?\s+from\s+['"].+['"]/gm, $:/^ *import\s+['"].+['"]/gm}, {getReplacement(l, h) {
...
const Ec = async(a, b, c, d) => {
  var e = await J(a);
  e = await Cc(e, {quoteProps:"dom", warn(f) {
  ...
  e = e.replace(/^import (['"])(.+?\.css)\1/gm, (f, g, k) => {
    try {
      ...
      wa(l) || B(l, `export default ${Dc.toString()}`);
      const h = v(t(a), k);
      k = `${k}.js`;
      const m = v(c, k);
      let n = w(t(m), l);
...
const Fc = ab("alamode"), Gc = (a, b) => a.includes(b) || a.some(c => b.startsWith(`${c}/`)), Y = async a => {
  const {input:b, g:c = ".", name:d, C:e = "-", noSourceMaps:f, extensions:g, debug:k, ca:l = d} = a;
  var h = v(c, d);
  a = "-" == e;
  ...
  await G(a);
  if (!Hc(h, g)) {
    return await Za({source:m, destination:a}), a;
  }
  const p = await jc({source:m, destination:a, debug:k, noSourceMaps:f});
  if ("-" != e) {
    var q = A(m);
    ({mode:q} = q);
    va(a, q);
    ...
    f || k || (h = xb({file:h, v:p, O:w(n || "", m)}), h = "/" + `/# sourceMappingURL=data:application/json;charset=utf-8;base64,${Buffer.from(h).toString("base64")}`, console.log("\n\n%s", h));
  }
  return a;
}, Jc = async a => {
  const {input:b, C:c, g:d = ".", M:e, P:f} = a, g = v(b, d), k = v(c, d), {content:l} = await I(g);
  await Object.keys(l).reduce(async(h, m) => {
    await h;
    h = v(g, m);
    var {type:n} = l[m];
    "File" == n ? Gc(a.l, v(d, m)) || (e && /jsx$/.test(m) ? (a.m && (n = m.replace(/jsx$/, "js"), h = await Y({...a, name:m, ca:n}), m = n), n = v(k, m), await Ic(h, f, c, n, {g:d, name:m, l:a.l, m:a.m})) : e && a.m ? await Y({...a, name:m}) : e ? await Ra(h, k) : await Y({...a, name:m})) : "Directory" == n && (m = v(d, m), await Jc({...a, g:m}));
  }, {});
}, Ic = async(a, b, c, d, {m:e, g:f, name:g, l:k} = {}) => {
  if (k && (f = v(f, g), Gc(k, f))) {
    return;
  }
  a = await Ec(a, b, c, e);
  ...
  await G(b);
  await Ya(b, a);
}, Hc = (a, b) => b.some(c => a.endsWith(c)), Kc = async a => {
  ...
  if (f.isDirectory()) {
    ...
    if (f.isFile()) {
      if (f = sa(b), d && /jsx$/.test(f)) {
        if (a = "-" == c ? "-" : v(c, f), f = await Ic(b, e, c, a), "-" == a) {
          return console.log(f);
        }
      } else {
        await Y({...a, input:t(b), g:"./", name:f});
      }
    }
  }
...
const Lc = async a => {
  ...
  const {input:b, g:c = "."} = a, d = v(b, c), {content:e} = await I(d);
  await Object.keys(e).reduce(async(f, g) => {
    await f;
    f = v(d, g);
    const {type:k} = e[g];
    "File" == k ? await Lc(f) : "Directory" == k && (g = v(c, g), await Mc({...a, g}));
  }, {});
};
async function Nc(a) {
  ({input:a} = a);
  ...
  const b = A(a);
  b.isDirectory() ? await Mc({input:a}) : b.isFile() && await Lc(a);
}
;function Oc() {
...
(async() => {
  try {
    ...
    if (ra) {
      return await Nc({input:ea, l:a, extensions:b});
    }
    let c = !1;
    oa ? c = "preact" : pa && (c = "@externs/preact");
    await Kc({input:ea, C:fa, noSourceMaps:ka, l:a, extensions:b, P:c, M:ma, debug:qa, m:na});
  } catch (a) {
