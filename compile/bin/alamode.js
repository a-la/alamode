...
const Ob = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Bb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  const h = Ab(a, this.config);
  this.J || (this.J = {});
  if (this.async) {
    return Nb(h, this.config, this.file, this.J).then(m => Lb(d, e, b, c, l, h, m));
  }
  a = Mb(h, this.config);
  return Lb(d, e, b, c, l, h, a);
...
const ec = [{re:/[\s\S]*/, replacement(a) {
  ...
  return a;
}}, {re:new RegExp(`^( *export\\s+?)( *${/(?:let|const|var|class|function\s*\*?|async +function)/.source}\\s+((?:${Qb.source}\\s*,?\\s*)+))`, "gm"), replacement:function(a, b, c, d) {
  d.split(/,\s*/).forEach(e => {
  ...
  return cc.call(this, b, l, d, g, c);
}}, {re:/^( *export\s+{([^}]+?)} *)(\n?)/gm, replacement:function(a, b, c, d) {
  a = Yb(c);
  a = Zb(a);
  this.emit("export", a);
  return this.noSourceMaps ? "" : `${V(b)}${d ? d : ""}`;
}}, {re:new RegExp(`^( *export\\s+default\\s+?)( *${/(?:class|function\s*\*?|async +function)/.source}\\s+(${Qb.source}))`, "m"), replacement:function(a, b, c, d) {
  a = d.trim();
  $b(a);
...
*/
const hc = {black:30, red:31, green:32, yellow:33, blue:34, magenta:35, cyan:36, white:37, grey:90};
function X(a, b) {
  return (b = hc[b]) ? `\x1b[${b}m${a}\x1b[0m` : a;
}
;const {Script:ic} = vm;
const jc = (a, b) => {
  const [c, , d] = a.split("\n");
...
}, oc = (a, b = {}, c = [], d = [], e = !1, f = null, g = {}, l = "") => {
  const k = nc(a), h = k ? a : `'${a}'`;
...
const pc = (a, b = []) => {
  ...
  var f = e.replace(/<\s*[^\s/>]+/, "");
  if (/\/\s*>$/.test(f)) {
    return a = f.replace(/\/\s*>$/, ""), f = "", new qc({i:e.replace(d.regExp, "=>"), h:a.replace(d.regExp, "=>"), content:"", tagName:b});
  }
  a = f.replace(/>$/, "");
  f = c + 1;
  c = !1;
  ...
  return new qc({i:k, h:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class qc {
  constructor(a) {
    this.i = a.i;
...
const wc = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {h:g = "", content:l, tagName:k, i:{length:h}} = rc(f);
  f = vc(l, c, d);
  const {v:m, s:n, j:p} = Z(g.replace(/^ */, ""));
  var q = oc(k, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  f = a.slice(0, e);
  a = a.slice(e + h);
  e = h - q.length;
  0 < e && (q = `${" ".repeat(e)}${q}`);
  ...
  if (e instanceof qc) {
    const {h:l = "", content:k, tagName:h} = e, {v:m, s:n} = Z(l);
    e = vc(k, b, c);
    e = oc(h, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {i:{length:l}, h:k = "", content:h, tagName:m} = rc(g), {v:n, s:p} = Z(k);
    g = vc(h, b, c);
    g = oc(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + l);
...
const zc = async(a, b, c) => {
  var d = await Va(a);
  d = await xc(d, {quoteProps:"dom", warn(e) {
  ...
  d = d.replace(/^import (['"])(.+?\.css)\1/gm, (e, f, g) => {
    try {
      ...
      ta(l) || A(l, `export default ${yc.toString()}`);
      const k = v(u(a), g);
      g = `${g}.js`;
      const h = v(c, g);
      let m = w(u(h), l);
...
const Ac = Za("alamode"), Cc = async a => {
  ...
  if (!f.includes(h) && !f.some(q => h.startsWith(`${q}/`))) {
    ...
    await E(m);
    if (Bc(h, l)) {
      var p = await gc({source:a, destination:m, debug:k, noSourceMaps:g});
  ...
  await Object.keys(k).reduce(async(h, m) => {
    ...
    h = v(l, m);
    const {type:p} = k[m];
    "File" == p ? e && /jsx$/.test(m) ? (m = await zc(n, f, c), h = h.replace(/jsx$/, "js"), await E(h), await Wa(h, m)) : e ? await Pa(n, l) : await Cc(Object.assign({}, a, {name:m})) : "Directory" == p && (m = v(d, m), await Dc(Object.assign({}, a, {B:m})));
  }, {});
}, Bc = (a, b) => b.some(c => a.endsWith(c)), Ec = async() => {
  ...
  if (f.isDirectory()) {
    ...
    f.isFile() && (f = pa(b), d && /jsx$/.test(f) ? (f = "-" == c ? "-" : v(c, f), a = await zc(b, e, c), "-" == f ? console.log(a) : (f = f.replace(/jsx$/, "js"), await E(f), await Wa(f, a))) : await Cc(Object.assign({}, a, {input:u(b), B:"./", name:f})));
  }
  "-" != c && process.stdout.write(`Transpiled code saved to ${c}\n`);
};
function Fc() {
  const {usage:a = {}, description:b, line:c, example:d} = {usage:Gc, description:"A tool to transpile JavaScript packages using regular expressions.\nSupports import/export and JSX transpilation.\nhttps://artdecocode.com/alamode/", line:"alamode source [-o destination] [-i dir,file] [-s] [-jp]", example:"alamode src -o build"};
  var e = Object.keys(a);
