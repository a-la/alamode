...
const ua = path.basename, u = path.dirname, w = path.join, va = path.parse, x = path.relative, wa = path.resolve;
const xa = fs.appendFileSync, ya = fs.chmodSync, y = fs.createReadStream, z = fs.createWriteStream, za = fs.existsSync, A = fs.lstat, B = fs.lstatSync, Aa = fs.mkdir, Ba = fs.readFileSync, Ca = fs.readdir, Da = fs.readlink, Ea = fs.symlink, D = fs.writeFileSync;
const Fa = (a, b = 0, c = !1) => {
  if (0 === b && !c) {
    return a;
  }
...
}, Ga = (a, b = !1) => Fa(a, 2 + (b ? 1 : 0)), Ha = a => {
  ({callee:{caller:a}} = a);
  return a;
};
const Ia = os.homedir;
const Ja = /\s+at.*(?:\(|\s)(.*)\)?/, Ka = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, La = Ia(), E = a => {
  ...
  }).filter(f => f.trim()).map(f => b ? f.replace(Ja, (g, k) => g.replace(k, k.replace(La, "~"))) : f).join("\n");
};
function Ma(a, b, c = !1) {
  return function(d) {
    var e = Ha(arguments), {stack:f} = Error();
    const g = Fa(f, 2, !0), k = (f = d instanceof Error) ? d.message : d;
...
const Ra = async(a, b) => {
  ...
  const {content:c} = await K(a), d = Object.keys(c).map(async e => {
    ...
    e = w(b, e);
    "Directory" == f ? await Ta(g, e) : "File" == f ? await Ra(g, e) : "SymbolicLink" == f && await Sa(g, e);
  });
  await Promise.all(d);
}, Ua = async(a, b) => {
  const c = await G(A, a), d = ua(a);
  b = w(b, d);
  c.isDirectory() ? await Ta(a, b) : c.isSymbolicLink() ? await Sa(a, b) : (await I(b), await Ra(a, b));
};
var Va = stream;
const Wa = stream.Transform, Xa = stream.Writable;
const Ya = (a, b) => {
  b.once("error", c => {
    a.emit("error", c);
  });
...
async function bb(a) {
  ...
  b && !d && (d = y(b));
  "-" == c ? d.pipe(process.stdout) : c ? await cb(c, d, b) : e instanceof Xa && (d.pipe(e), await new Promise((f, g) => {
    e.on("error", g);
    e.on("finish", f);
  }));
}
const cb = async(a, b, c) => {
  if (b.path == a || c == a) {
    ({b:c} = new Za({rs:b}));
    const d = await c;
    await new Promise((e, f) => {
...
const fb = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".split("");
function O(a) {
  var b = "";
  a = 0 > a ? (-a << 1) + 1 : a << 1;
  do {
...
const ib = /^[A-Za-z0-9+\-.]+:/;
function R(a) {
  return "/" == a[0] ? "/" == a[1] ? "scheme-relative" : "path-absolute" : ib.test(a) ? "absolute" : "path-relative";
}
function jb(a, b) {
...
const kb = function(a) {
  return b => {
    ...
    a(b);
    b = b.toString();
    return "absolute" === c ? b : "scheme-relative" === c ? b.slice(5) : "path-absolute" === c ? b.slice(11) : jb(d, b);
  };
}(() => {
});
function lb(a, b) {
  a: {
    if (R(a) !== R(b)) {
      ...
      c = c.protocol !== a.protocol || c.user !== a.user || c.password !== a.password || c.hostname !== a.hostname || c.port !== a.port ? null : jb(a, c);
    }
  }
  return "string" == typeof c ? c : kb(b);
}
;class mb {
  constructor() {
    this.a = [];
...
class qb {
  ...
  toJSON() {
    ...
    this.a && (b.sourcesContent = pb(this, a, this.b));
    return b;
  }
  toString() {
...
function T(a, b) {
  function c() {
    return b.filter(rb).reduce((d, {re:e, replacement:f}) => {
      if (this.c) {
        return d;
      }
      if ("string" == typeof f) {
...
;const sb = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), tb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, U = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
    const {getReplacement:f = tb, getRegex:g = sb} = b || {}, k = g(d);
...
class wb extends Wa {
  constructor(a, b) {
    super(b);
    this.rules = (Array.isArray(a) ? a : [a]).filter(rb);
    this.c = !1;
    this.a = b;
  }
  async replace(a, b) {
    const c = new wb(this.rules, this.a);
    b && Object.assign(c, b);
    a = await ub(c, a);
    c.c && this.brake();
...
;const xb = /\/\*(?:[\s\S]+?)\*\//g, yb = /\/\/(.+)/gm;
const Ab = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:k} = U({comments:xb, inlineComments:yb, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), l = [b, c, d, e, f, g, k], [h, m, n, p, q, t, v] = l.map(W), [C, H, bc, cc, dc, ec, fc] = l.map(gc => V(gc));
  return {rules:[q, h, m, n, v, t, p, zb, ...a, cc, ec, fc, bc, H, C, dc], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:k}};
}, zb = {re:/./, replacement(a) {
  this.stopProcessing && this.brake();
  return a;
}};
const Bb = ({file:a, s:b, M:c, sourceRoot:d}) => {
  const e = new qb({file:a, sourceRoot:d});
  b.replace(xb, (f, g) => {
    if ("\n" == b[g + f.length]) {
      return "\n".repeat(f.split("\n").length - 1);
    }
    ...
    g = " ".repeat(g[f].length);
    return `${"\n".repeat(f)}${g}`;
  }).replace(yb, f => " ".repeat(f.length)).split("\n").forEach((f, g) => {
    const k = g + 1;
...
function Cb({source:a, $:b, name:c, destination:d, file:e, s:f}) {
  a = x(b, a);
  e = Bb({file:e, s:f, M:a});
  c = `${c}.map`;
  xa(d, `\n//# sourceMappingURL=${c}`);
...
;const Db = a => `if (${a} && ${a}.__esModule) ${a} = ${a}.default`, Eb = (a, b, c, d = null) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (e, f, g) => `${f}=${g}`)}${d ? d : "r" + `equire(${b}${c}${b})`}`, Fb = (a, b = {}) => {
  if (!b.import) {
    return a;
  }
...
}, Gb = /(\s+from\s+)(?:%%_RESTREAM_STRINGS_REPLACEMENT_(\d+)_%%|%%_RESTREAM_LITERALS_REPLACEMENT_(\d+)_%%)/, Hb = (a = {import:{}}) => {
  try {
...
const Kb = async a => {
  ...
  if (!b) {
    ...
    if (b.isDirectory()) {
      b = !1;
      let e;
      a.endsWith("/") || (e = c = await Jb(a), b = !0);
      if (!e) {
        c = await Jb(w(a, "index"));
        if (!c) {
...
const Pb = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), Qb = (a, b, c = {}) => {
  ...
  if (/^[./]/.test(a) || Ob.includes(a) || Ib(a, b)) {
    return !0;
  }
}, Vb = async(a, b, c, d) => {
  if (Ub(a, b)) {
  ...
  if (c) {
    try {
      const {alamode:e} = await Nb(u(c), a, {fields:["alamode"]});
      d[a] = !!e;
      return e;
    } catch (e) {
      return !1;
  ...
  b = b ? {d:`${b} = ${"r" + `equire(${c}${d}${c})`}`, v:Db(b)} : void 0;
  const {d:f, v:g} = b;
...
const Wb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Gb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  const h = (f = Qb(this.file, l, this.config)) || Fb(a, this.config);
  f && (d ? b && (d = d.replace(/{/, `{ ${c},`), d = b.replace(/\S/g, " ") + d, c = b = void 0) : (d = b.replace(/(\S+)/, "{ $1 }"), c = b = void 0));
  this.J || (this.J = {});
  if (this.async) {
    return Vb(h, this.config, this.file, this.J).then(m => Tb(d, e, b, c, k, h, m));
  }
  a = Ub(h, this.config);
  ...
  b = Fb(f, this.config);
  return a + `${e}${b}${e});`;
}}, {re:new RegExp(`${/( *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+(.+?))/.source}${Gb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  const [, k, l] = /(["'`])(.+?)\1/.exec(g ? this.markers.literals.map[g] : this.markers.strings.map[f]);
  a = Fb(l, this.config);
  e = Eb(e, k, a);
  ({length:b} = b.split("\n"));
  b = "\n".repeat(b - 1);
  a = /^[./]/.test(a) && !Hb(this.config);
  return `${c ? [`${b}${a ? "const" : "let"} ${d} = ${c}${e}`, ...a ? [] : [Db(c)]].join("; ") : `${b}const ${d}${e}`};`;
}}];
...
const ic = (a, b, c) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (d, e, f) => `${e}=${f}`)}${"r" + `equire(${b}${c}${b});`}`;
...
const nc = async({source:a, destination:b, writable:c, debug:d, noSourceMaps:e}) => {
  const f = new mc(a);
  e && (f.noSourceMaps = e);
  d && (f.stopProcessing = !0);
  d = await L(a);
  f.end(d);
  await Promise.all([bb({source:a, ...c ? {writable:c} : {destination:b}, readable:f}), new Promise((g, k) => f.on("finish", g).on("error", k))]);
  return d;
};
/*
...
const yc = (a, b = []) => {
  ...
  var f = e.replace(/<\s*[^\s/>]+/, "");
  if (/\/\s*>$/.test(f)) {
    return a = f.replace(/\/\s*>$/, ""), f = "", new zc({h:e.replace(d.regExp, "=>"), g:a.replace(d.regExp, "=>"), content:"", tagName:b});
  }
  a = f.replace(/>$/, "");
  f = c + 1;
  c = !1;
  ...
  return new zc({h:l, g:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class zc {
  constructor(a) {
    this.h = a.h;
...
const Fc = (a, b = {}) => {
  var c = b.quoteProps, d = b.warn, e = rc(a);
  if (null === e) {
    return a;
  }
  var f = a.slice(e);
  const {g = "", content:k, tagName:l, h:{length:h}} = Ac(f);
  f = Ec(k, c, d);
  const {w:m, u:n, l:p} = uc(g.replace(/^ */, ""));
  d = xc(l, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  c = a.slice(0, e);
  a = a.slice(e + h);
  e = h - d.length;
  0 < e && (d = `${" ".repeat(e)}${d}`);
  ...
  if (e instanceof zc) {
    const {g:k = "", content:l, tagName:h} = e, {w:m, u:n} = uc(k);
    e = Ec(l, b, c);
    e = xc(h, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {h:{length:k}, g:l = "", content:h, tagName:m} = Ac(g), {w:n, u:p} = uc(l);
    g = Ec(h, b, c);
    g = xc(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + k);
...
const Ic = async(a, b, c, d) => {
  var e = await L(a);
  e = await Gc(e, {quoteProps:"dom", warn(f) {
  ...
  e = e.replace(/^import (['"])(.+?\.css)\1/gm, (f, g, k) => {
    try {
      ...
      za(l) || D(l, `export default ${Hc.toString()}`);
      const h = w(u(a), k);
      k = `${k}.js`;
      const m = w(c, k);
      let n = x(u(m), l);
...
const Jc = db("alamode"), Kc = (a, b) => a.includes(b) || a.some(c => b.startsWith(`${c}/`)), Z = async a => {
  ...
  await I(a);
  if (!Lc(h, g)) {
    return await bb({source:m, destination:a}), a;
  }
  const p = await nc({source:m, destination:a, debug:k, noSourceMaps:f});
  if ("-" != e) {
    {
      const q = B(m).mode;
      ya(a, q);
    ...
    Cb({destination:a, file:h, name:d, $:n, source:m, s:p});
  } else {
    f || k || (h = Bb({file:h, s:p, M:x(n || "", m)}), h = "/" + `/# sourceMappingURL=data:application/json;charset=utf-8;base64,${Buffer.from(h).toString("base64")}`, console.log("\n\n%s", h));
  }
  return a;
}, Nc = async a => {
  ...
  await Object.keys(l).reduce(async(h, m) => {
    ...
    h = w(g, m);
    var {type:n} = l[m];
    "File" == n ? Kc(a.ignore, w(d, m)) || (e && /jsx$/.test(m) ? (a.i && (n = m.replace(/jsx$/, "js"), h = await Z({...a, name:m, aa:n}), m = n), n = w(k, m), await Mc(h, f, c, n, {j:d, name:m, ignore:a.ignore, i:a.i})) : e && a.i ? await Z({...a, name:m}) : e ? await Ua(h, k) : await Z({...a, name:m})) : "Directory" == n && (m = w(d, m), await Nc({...a, j:m}));
  }, {});
}, Mc = async(a, b, c, d, {i:e, j:f, name:g, ignore:k} = {}) => {
  if (k && (f = w(f, g), Kc(k, f))) {
    return;
  }
  a = await Ic(a, b, c, e);
  ...
  if (f.isDirectory()) {
    ...
    if (f.isFile()) {
      if (f = ua(b), d && /jsx$/.test(f)) {
        if (a = "-" == c ? "-" : w(c, f), f = await Mc(b, e, c, a), "-" == a) {
...
const Pc = async a => {
  var b = await L(a);
  const c = new wb([{re:/^ *(?:var|let|const)\s+(\S+?)(\s*)=(\s*)require\((['"])(.+?)\4\)/gm, replacement(d, e, f, g, k, l) {
...
async function Qc(a) {
  ...
  var b = B(a);
  b.isDirectory() ? ({content:b} = await K(a), a = Qa(b, a), await Promise.all(a.map(async c => {
    await Pc(c);
  }))) : b.isFile() && await Pc(a);
}
;function Rc(a = {usage:{}}) {
...
(async() => {
  try {
    ...
    if (ma) {
      return await Qc({input:fa, ignore:a, extensions:b});
    }
    let c = !1;
    sa ? c = "preact" : ta && (c = "@externs/preact");
    await Oc({input:fa, A:ha, noSourceMaps:ja, ignore:a, extensions:b, N:c, K:qa, debug:la, i:ra});
  } catch (a) {
