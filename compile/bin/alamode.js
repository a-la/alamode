...
const zb = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:k} = U({comments:wb, inlineComments:xb, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), l = [b, c, d, e, f, g, k], [h, m, n, p, q, t, v] = l.map(W), [C, H, $b, ac, bc, cc, dc] = l.map(ec => V(ec));
  return {rules:[q, h, m, n, v, t, p, yb, ...a, ac, cc, dc, $b, H, C, bc], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:k}};
}, yb = {re:/./, replacement(a) {
...
const Ub = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Fb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
  ...
  ({length:b} = b.split("\n"));
  b = "\n".repeat(b - 1);
  a = /^[./]/.test(a) && !Gb(this.config);
  return `${c ? [`${b}${a ? "const" : "let"} ${d} = ${c}${e}`, ...a ? [] : [Cb(c)]].join("; ") : `${b}const ${d}${e}`};`;
}}];
