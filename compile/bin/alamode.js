...
const sa = path.basename, u = path.dirname, w = path.join, x = path.relative, ta = path.resolve;
const ua = fs.appendFileSync, va = fs.chmodSync, y = fs.createReadStream, z = fs.createWriteStream, wa = fs.existsSync, A = fs.lstat, B = fs.lstatSync, xa = fs.mkdir, ya = fs.readFileSync, za = fs.readdir, Aa = fs.readlink, Ba = fs.symlink, D = fs.writeFileSync;
const Ca = (a, b = 0, c = !1) => {
  if (0 === b && !c) {
    return a;
  }
...
const Oa = async(a, b) => {
  ...
  b = w(b, d);
  c.isDirectory() ? await Qa(a, b) : c.isSymbolicLink() ? await Pa(a, b) : (await I(b), await Oa(a, b));
};
var Sa = stream;
const Ta = stream.Transform, Ua = stream.Writable;
const Va = (a, b) => {
  b.once("error", c => {
    a.emit("error", c);
  });
...
;const pb = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), qb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, U = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
...
const xb = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:k} = U({comments:ub, inlineComments:vb, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), l = [b, c, d, e, f, g, k], [h, m, n, p, q, t, v] = l.map(W), [C, G, Xb, Yb, Zb, $b, ac] = l.map(bc => V(bc));
  return {rules:[q, h, m, n, v, t, p, wb, ...a, Yb, $b, ac, Xb, G, C, Zb], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:k}};
}, wb = {re:/./, replacement(a) {
  this.stopProcessing && this.brake();
  return a;
}};
const yb = ({file:a, s:b, M:c, sourceRoot:d}) => {
...
function zb({source:a, $:b, name:c, destination:d, file:e, s:f}) {
  a = x(b, a);
  e = yb({file:e, s:f, M:a});
  c = `${c}.map`;
  ua(d, `\n//# sourceMappingURL=${c}`);
...
const Hb = async a => {
  ...
  if (!b) {
    ...
    if (b.isDirectory()) {
      b = !1;
      let e;
      a.endsWith("/") || (e = c = await Gb(a), b = !0);
      if (!e) {
        c = await Gb(w(a, "index"));
        if (!c) {
...
const Lb = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), Mb = (a, b, c = {}) => {
  ...
  if (/^[./]/.test(a) || Kb.includes(a) || Fb(a, b)) {
    return !0;
  }
}, Rb = async(a, b, c, d) => {
  if (Qb(a, b)) {
  ...
  if (c) {
    try {
      const {alamode:e} = await Jb(u(c), a, {fields:["alamode"]});
      d[a] = !!e;
      return e;
    } catch (e) {
      return !1;
  ...
  b = b ? {d:`${b} = ${"r" + `equire(${c}${d}${c})`}`, v:Ab(b)} : void 0;
  const {d:f, v:g} = b;
...
const Sb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Db.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
...
const hc = [{re:/[\s\S]*/, replacement(a) {
  ...
  return a;
}}, {re:new RegExp(`^( *export\\s+?)( *${/(?:let|const|var|class|function\s*\*?|async +function)/.source}\\s+((?:${Ub.source}\\s*,?\\s*)+))`, "gm"), replacement:function(a, b, c, d) {
  d.split(/,\s*/).forEach(e => {
...
const uc = (a, b = []) => {
  ...
  var f = e.replace(/<\s*[^\s/>]+/, "");
  if (/\/\s*>$/.test(f)) {
    return a = f.replace(/\/\s*>$/, ""), f = "", new vc({h:e.replace(d.regExp, "=>"), g:a.replace(d.regExp, "=>"), content:"", tagName:b});
  }
  a = f.replace(/>$/, "");
  f = c + 1;
  c = !1;
  ...
  return new vc({h:l, g:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class vc {
  constructor(a) {
    this.h = a.h;
...
const Bc = (a, b = {}) => {
  var c = b.quoteProps, d = b.warn, e = nc(a);
  if (null === e) {
    return a;
  }
  var f = a.slice(e);
  const {g = "", content:k, tagName:l, h:{length:h}} = wc(f);
  f = Ac(k, c, d);
  const {w:m, u:n, l:p} = qc(g.replace(/^ */, ""));
  d = tc(l, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  c = a.slice(0, e);
  a = a.slice(e + h);
  e = h - d.length;
  0 < e && (d = `${" ".repeat(e)}${d}`);
  ...
  if (e instanceof vc) {
    const {g:k = "", content:l, tagName:h} = e, {w:m, u:n} = qc(k);
    e = Ac(l, b, c);
    e = tc(h, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {h:{length:k}, g:l = "", content:h, tagName:m} = wc(g), {w:n, u:p} = qc(l);
    g = Ac(h, b, c);
    g = tc(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + k);
...
const Ec = async(a, b, c, d) => {
  ...
  e = e.replace(/^import (['"])(.+?\.css)\1/gm, (f, g, k) => {
    try {
      ...
      wa(l) || D(l, `export default ${Dc.toString()}`);
      const h = w(u(a), k);
      k = `${k}.js`;
      const m = w(c, k);
      let n = x(u(m), l);
...
const Fc = ab("alamode"), Gc = (a, b) => a.includes(b) || a.some(c => b.startsWith(`${c}/`)), Z = async a => {
  ...
  if ("-" != e) {
    ...
    zb({destination:a, file:h, name:d, $:n, source:m, s:p});
  } else {
    f || k || (h = yb({file:h, s:p, M:x(n || "", m)}), h = "/" + `/# sourceMappingURL=data:application/json;charset=utf-8;base64,${Buffer.from(h).toString("base64")}`, console.log("\n\n%s", h));
  }
  return a;
}, Jc = async a => {
  ...
  await Object.keys(l).reduce(async(h, m) => {
    ...
    h = w(g, m);
    var {type:n} = l[m];
    "File" == n ? Gc(a.ignore, w(d, m)) || (e && /jsx$/.test(m) ? (a.i && (n = m.replace(/jsx$/, "js"), h = await Z({...a, name:m, aa:n}), m = n), n = w(k, m), await Ic(h, f, c, n, {j:d, name:m, ignore:a.ignore, i:a.i})) : e && a.i ? await Z({...a, name:m}) : e ? await Ra(h, k) : await Z({...a, name:m})) : "Directory" == n && (m = w(d, m), await Jc({...a, j:m}));
  }, {});
}, Ic = async(a, b, c, d, {i:e, j:f, name:g, ignore:k} = {}) => {
  if (k && (f = w(f, g), Gc(k, f))) {
    return;
  }
  a = await Ec(a, b, c, e);
  ...
  if (f.isDirectory()) {
    ...
    if (f.isFile()) {
      if (f = sa(b), d && /jsx$/.test(f)) {
        if (a = "-" == c ? "-" : w(c, f), f = await Ic(b, e, c, a), "-" == a) {
...
const Lc = async a => {
  var b = await L(a);
  const c = new tb([{re:/^ *(?:var|let|const)\s+(\S+?)(\s*)=(\s*)require\((['"])(.+?)\4\)/gm, replacement(d, e, f, g, k, l) {
...
async function Mc(a) {
  ...
  var b = B(a);
  b.isDirectory() ? ({content:b} = await K(a), a = Na(b, a), await Promise.all(a.map(async c => {
    await Lc(c);
  }))) : b.isFile() && await Lc(a);
}
;function Nc() {
...
(async() => {
  try {
    ...
    if (ra) {
      return await Mc({input:ea, ignore:a, extensions:b});
    }
    let c = !1;
    oa ? c = "preact" : pa && (c = "@externs/preact");
    await Kc({input:ea, A:fa, noSourceMaps:ka, ignore:a, extensions:b, N:c, K:ma, debug:qa, i:na});
  } catch (a) {
