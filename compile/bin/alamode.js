...
boolean:!0, short:"p"}, "preact-externs":{description:'Same as `preact`, but imports from `\uff20externs/preact`\n`import { h } from "\uff20externs/preact"`.', boolean:!0, short:"E"}, debug:{description:"Will make \u00c0LaMode stop after replacing markers.", boolean:!0, short:"d"}, require:{description:"Renames `require` calls into imports, and `module.exports` assignments to exports.\nGreat for refactoring older code.", boolean:!0, short:"r"}}, r = function(a = {}, b = process.argv) {
  ...
  return Object.keys(a).reduce(({i:e, ...f}, g) => {
    ...
    let l;
    if ("string" == typeof k) {
      ({value:l, argv:e} = aa(e, g, k));
    } else {
      try {
        const {short:h, boolean:m, number:n, command:p, multiple:q} = k;
        p && q && c.length ? (l = c, d = !0) : p && c.length ? (l = c[0], d = !0) : {value:l, argv:e} = aa(e, g, h, m, n);
      } catch (h) {
        return {i:e, ...f};
      }
    }
    return void 0 === l ? {i:e, ...f} : {i:e, ...f, [g]:l};
  }, {i:b});
...
const {basename:sa, dirname:t, join:v, relative:w, resolve:ta} = path;
const {appendFileSync:ua, chmodSync:va, createReadStream:x, createWriteStream:y, existsSync:wa, lstat:z, lstatSync:A, mkdir:xa, readFileSync:ya, readdir:za, readlink:Aa, symlink:Ba, writeFileSync:B} = fs;
const Ca = (a, b = 0, c = !1) => {
  if (0 === b && !c) {
    return a;
  }
...
const {homedir:Fa} = os;
const Ga = /\s+at.*(?:\(|\s)(.*)\)?/, Ha = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, Ia = Fa(), C = a => {
...
;async function La(a, b) {
  b = b.map(async c => {
    const d = v(a, c);
    return {lstat:await E(z, d), path:d, relativePath:c};
  });
...
;const Pa = async(a, b) => {
  ...
  await G(v(b, "path.file"));
  const {content:c} = await Oa(a), d = Object.keys(c).map(async e => {
  ...
  const c = await E(z, a), d = sa(a);
  b = v(b, d);
  c.isDirectory() ? await Ra(a, b) : c.isSymbolicLink() ? await Qa(a, b) : (await G(b), await Pa(a, b));
};
var Ta = stream;
...
class Xa extends Va {
  constructor(a) {
    const {binary:b = !1, rs:c = null, ...d} = a || {}, {V:e = D(!0), proxyError:f} = a || {}, g = (k, l) => e(l);
    super(d);
    this.a = [];
...
const ab = a => a.lstat.isDirectory(), bb = a => !a.lstat.isDirectory();
async function J(a) {
  if (!a) {
    throw Error("Please specify a path to the directory");
  }
...
async function db(a) {
  ...
  b && !d && (d = x(b));
  "-" == c ? d.pipe(process.stdout) : c ? await eb(c, d, b) : e instanceof Va && (d.pipe(e), await new Promise((f, g) => {
    e.on("error", g);
    e.on("finish", f);
  }));
}
const eb = async(a, b, c) => {
  if (b.path == a || c == a) {
    ({b:c} = new Xa({rs:b}));
    const d = await c;
    await new Promise((e, f) => {
...
const hb = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".split("");
function M(a) {
  var b = "";
  a = 0 > a ? (-a << 1) + 1 : a << 1;
  do {
...
function lb(a, b) {
  "string" == typeof a && (a = new N(a));
  "string" == typeof b && (b = new N(b));
...
const mb = function(a) {
  return b => {
    ...
    a(b);
    b = b.toString();
    return "absolute" === c ? b : "scheme-relative" === c ? b.slice(5) : "path-absolute" === c ? b.slice(11) : lb(d, b);
  };
}(() => {
});
function nb(a, b) {
  a: {
    if (P(a) !== P(b)) {
      ...
      c = c.protocol !== a.protocol || c.user !== a.user || c.password !== a.password || c.hostname !== a.hostname || c.port !== a.port ? null : lb(a, c);
    }
  }
  return "string" == typeof c ? c : mb(b);
}
;class ob {
  constructor() {
    this.a = [];
...
class sb {
  ...
  toJSON() {
    ...
    var b = this.f.a.slice();
    {
      ...
      let p;
      var d = this.J;
      d.b || (d.a.sort(O), d.b = !0);
      d = d.a;
      for (let q = 0, u = d.length; q < u; q++) {
        var e = d[q];
        n = "";
        ...
        n += M(e.generatedColumn - c);
        c = e.generatedColumn;
        null != e.source && (p = this.o.indexOf(e.source), n += M(p - h), h = p, n += M(e.originalLine - 1 - k), k = e.originalLine - 1, n += M(e.originalColumn - g), g = e.originalColumn, null != e.name && (e = this.f.indexOf(e.name), n += M(e - l), l = e));
        m += n;
      }
      c = m;
    }
    ...
    this.a && (b.sourcesContent = rb(this, a, this.b));
    return b;
  }
  toString() {
...
function R(a, b) {
  function c() {
    return b.filter(tb).reduce((d, {re:e, replacement:f}) => {
      if (this.c) {
        return d;
      }
      if ("string" == typeof f) {
...
;const ub = a => new RegExp(`%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_(\\d+)_%%`, "g"), vb = (a, b) => `%%_RESTREAM_${a.toUpperCase()}_REPLACEMENT_${b}_%%`, S = (a, b) => Object.keys(a).reduce((c, d) => {
  {
    var e = a[d];
    const {getReplacement:f = vb, getRegex:g = ub} = b || {}, k = g(d);
...
class V extends Ua {
  constructor(a, b) {
    super(b);
    this.rules = (Array.isArray(a) ? a : [a]).filter(tb);
    this.c = !1;
    this.a = b;
  }
  async replace(a, b) {
    const c = new V(this.rules, this.a);
    b && Object.assign(c, b);
    a = await wb(c, a);
    c.c && this.brake();
...
;const yb = /\/\*(?:[\s\S]+?)\*\//g, zb = /\/\/(.+)/gm;
const Bb = (a = []) => {
  const {comments:b, inlineComments:c, strings:d, literals:e, escapes:f, regexes:g, regexGroups:k} = S({comments:yb, inlineComments:zb, strings:/(["'])(.*?)\1/gm, literals:/`([\s\S]*?)`/gm, escapes:/\\[\\`'"/]/g, regexes:/\/(.+?)\//gm, regexGroups:/\[(.*?)\]/gm}), l = [b, c, d, e, f, g, k], [h, m, n, p, q, u, F] = l.map(U), [Z, ac, bc, cc, dc, ec, fc] = l.map(gc => T(gc));
  return {rules:[q, h, m, n, F, u, p, Ab, ...a, cc, ec, fc, bc, ac, Z, dc], markers:{literals:e, strings:d, comments:b, inlineComments:c, escapes:f, regexes:g, regexGroups:k}};
}, Ab = {re:/./, replacement(a) {
  this.stopProcessing && this.brake();
  return a;
}};
const Cb = ({file:a, u:b, N:c, sourceRoot:d}) => {
  const e = new sb({file:a, sourceRoot:d});
  b.replace(yb, (f, g) => {
    if ("\n" == b[g + f.length]) {
      return "\n".repeat(f.split("\n").length - 1);
    }
    ...
    g = " ".repeat(g[f].length);
    return `${"\n".repeat(f)}${g}`;
  }).replace(zb, f => " ".repeat(f.length)).split("\n").forEach((f, g) => {
    const k = g + 1;
    f.replace(/(?:(?:\s+)|(?:[$_\w\d]+)|.)/g, (l, h) => {
      if (0 != h || !/^\s+$/.test(l)) {
        l = {line:k, column:h};
        {
          let {I:m, M:n = null, source:p = null, name:q = null} = {I:l, source:c, M:l};
          if (!m) {
...
function Db({source:a, aa:b, name:c, destination:d, file:e, u:f}) {
  a = w(b, a);
  e = Cb({file:e, u:f, N:a});
  c = `${c}.map`;
  ua(d, `\n//# sourceMappingURL=${c}`);
...
;const Eb = a => `if (${a} && ${a}.__esModule) ${a} = ${a}.default`, Fb = (a, b, c, d = null) => `${a.replace(/(\s+)from(\s+)([\s\S])*/, (e, f, g) => `${f}=${g}`)}${d ? d : "r" + `equire(${b}${c}${b})`}`, Gb = (a, b = {}) => {
  if (!b.import) {
    return a;
  }
...
}, Hb = /(\s+from\s+)(?:%%_RESTREAM_STRINGS_REPLACEMENT_(\d+)_%%|%%_RESTREAM_LITERALS_REPLACEMENT_(\d+)_%%)/, Ib = (a = {import:{}}) => {
  try {
...
const Lb = async a => {
  ...
  if (!b) {
    ...
    if (b.isDirectory()) {
      b = !1;
      let e;
      a.endsWith("/") || (e = c = await Kb(a), b = !0);
      if (!e) {
        c = await Kb(v(a, "index"));
        if (!c) {
...
const Nb = async(a, b, c = {}) => {
  ...
  if (g) {
    ...
    return {entry:w("", k), packageJson:w("", f), ...l ? {version:l} : {}, packageName:h, ...m ? {hasMain:!0} : {}, ...n ? {} : {entryExists:!1}, ...p};
  }
  if ("/" == a && !g) {
    throw Error(`Package.json for module ${b} not found.`);
  }
  return Nb(v(ta(a), ".."), b, c);
}, Mb = async(a, b = []) => {
  const c = await I(a);
  let d, e, f, g, k;
  try {
  ...
  let l;
  try {
    ({path:l} = await Lb(a)), a = l;
  } catch (h) {
  }
  return {entry:a, version:e, packageName:f, main:!d && g, entryExists:!!l, ...k};
};
const {builtinModules:Ob} = _module;
const Pb = a => a.replace(/(\s+)as(\s+)/g, (b, c, d) => `${1 == c.split("\n").length ? "" : c}:${d}`), Qb = (a, b, c = {}) => {
  if (!c.import) {
    return null;
  }
  ({import:{stdlib:c}} = c);
  if (c) {
    ...
    return d.includes(b) ? w(t(a), e).replace(/.js$/, "") : null;
  }
  return null;
}, Tb = (a, b, c, d, e, f, g) => {
  const {t:k, w:l} = Rb(c, d, e, f, g);
  a = Sb(a, b, e, f, d);
  ...
  if (/^[./]/.test(a) || Ob.includes(a) || Jb(a, b)) {
    return !0;
  }
}, Vb = async(a, b, c, d) => {
  if (Ub(a, b)) {
  ...
  if (c) {
    try {
      const {alamode:e} = await Nb(t(c), a, {fields:["alamode"]});
      d[a] = !!e;
      return e;
    } catch (e) {
      return !1;
  ...
  b = b ? {d:`${b} = ${"r" + `equire(${c}${d}${c})`}`, w:Eb(b)} : void 0;
  const {d:f, w:g} = b;
...
const Wb = [{re:new RegExp(`${/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?/.source}${Hb.source}`, "gm"), replacement:function(a, b, c, d, e, f, g) {
...
const mc = [{re:/[\s\S]*/, replacement(a) {
  ...
  return this.noSourceMaps ? c : `${W(b, !1)}${c}`;
  ...
  return kc.call(this, b, k, d, g, c);
}}, {re:/^( *export\s+{([^}]+?)} *)(\n?)/gm, replacement:function(a, b, c, d) {
  a = Zb(c);
  a = $b(a);
  this.emit("export", a);
  return this.noSourceMaps ? "" : `${W(b)}${d ? d : ""}`;
...
const oc = async({source:a, destination:b, writable:c, debug:d, noSourceMaps:e}) => {
  const f = new nc(a);
  e && (f.noSourceMaps = e);
  d && (f.stopProcessing = !0);
  d = await I(a);
  f.end(d);
  await Promise.all([db({source:a, ...c ? {writable:c} : {destination:b}, readable:f}), new Promise((g, k) => f.on("finish", g).on("error", k))]);
  return d;
};
/*
...
const tc = a => {
  ...
  var k = c.reduce((l, {open:h, close:m}) => {
    l = a.slice(l, h);
    const [, n, p, q, u] = /(\s*)(\S+)(\s*)=(\s*)$/.exec(l) || [];
    h = a.slice(h + 1, m);
    if (!p && !/\s*\.\.\./.test(h)) {
      throw Error("Could not detect prop name");
    }
    p ? e[p] = h : f.push(h);
    g[p] = {before:n, F:q, D:u};
    h = l || "";
  ...
  a.replace(/(\s*)(\S+)(\s*)=(\s*)(["'])([\s\S]+?)\5/g, (d, e, f, g, k, l, h, m) => {
    c[f] = {before:e, F:g, D:k};
    b.push({s:m, name:f, P:`${l}${h}${l}`});
    return "%".repeat(d.length);
  }).replace(/(\s*)([^\s%]+)/g, (d, e, f, g) => {
    c[f] = {before:e};
    b.push({s:g, name:f, P:"true"});
  });
  return {C:[...b.reduce((d, {s:e, name:f, P:g}) => {
    d[e] = [f, g];
    return d;
  ...
  return g || b.length ? `{${f.reduce((k, l) => {
    const h = a[l], m = c || -1 != l.indexOf("-") ? `'${l}'` : l, {before:n = "", F:p = "", D:q = ""} = d[l] || {};
    return [...k, `${n}${m}${p}:${q}${h}`];
  }, b).join(",")}${e}}` : "{}";
}, xc = (a = "") => {
  [a] = a;
...
}, yc = (a, b = {}, c = [], d = [], e = !1, f = null, g = {}, k = "") => {
  const l = xc(a), h = l ? a : `'${a}'`;
...
const zc = (a, b = []) => {
  ...
  c = !1;
  let g = 1, k;
  R(l, [{re:new RegExp(`[\\s\\S](?:<\\s*${b}(\\s+|>)|/\\s*${b}\\s*>)`, "g"), replacement(h, m, n, p) {
    if (c) {
      return h;
    }
    m = !m && h.endsWith(">");
  ...
  return new Ac({h:l, g:a.replace(d.regExp, "=>"), content:f.replace(d.regExp, "=>"), tagName:b});
};
class Ac {
  constructor(a) {
    this.h = a.h;
...
;const Cc = a => {
  ...
  let c = 0;
  b = b.reduce((d, {from:e, to:f, W:g, X:k}) => {
...
const Gc = (a, b = {}) => {
  ...
  var f = a.slice(e);
  const {g = "", content:k, tagName:l, h:{length:h}} = Bc(f);
  f = Fc(k, c, d);
  const {A:m, v:n, m:p} = vc(g.replace(/^ */, ""));
  var q = yc(l, m, f, n, c, d, p, /\s*$/.exec(g) || [""]);
  f = a.slice(0, e);
  a = a.slice(e + h);
  e = h - q.length;
  0 < e && (q = `${" ".repeat(e)}${q}`);
  ...
  if (e instanceof Ac) {
    const {g:k = "", content:l, tagName:h} = e, {A:m, v:n} = vc(k);
    e = Fc(l, b, c);
    e = yc(h, m, e, n, b, c);
    return [...d, e];
  }
  ...
  if (f) {
    var g = e.slice(f);
    const {h:{length:k}, g:l = "", content:h, tagName:m} = Bc(g), {A:n, v:p} = vc(l);
    g = Fc(h, b, c);
    g = yc(m, n, g, p, b, c);
    const q = e.slice(0, f);
    e = e.slice(f + k);
...
const Hc = (a, b = {}) => {
  const {e:c, T:d, U:e, s:f, Y:g, Z:k} = S({T:/^ *export\s+default\s+{[\s\S]+?}/mg, e:/^ *export\s+(?:default\s+)?/mg, U:/^ *export\s+{[^}]+}\s+from\s+(['"])(?:.+?)\1/mg, s:/^ *import(\s+([^\s,]+)\s*,?)?(\s*{(?:[^}]+)})?\s+from\s+['"].+['"]/gm, Y:/^ *import\s+(?:(.+?)\s*,\s*)?\*\s+as\s+.+?\s+from\s+['"].+['"]/gm, Z:/^ *import\s+['"].+['"]/gm}, {getReplacement(l, h) {
    return `/*%%_RESTREAM_${l.toUpperCase()}_REPLACEMENT_${h}_%%*/`;
  }, getRegex(l) {
    return new RegExp(`/\\*%%_RESTREAM_${l.toUpperCase()}_REPLACEMENT_(\\d+)_%%\\*/`, "g");
  }});
  a = R(a, [U(e), U(d), U(c), U(f), U(g), U(k)]);
  b = Gc(a, b);
  return R(b, [T(e), T(d), T(c), T(f), T(g), T(k)]);
};
function Ic(a = "") {
  if (document) {
...
const Jc = async(a, b, c, d) => {
  var e = await I(a);
  e = await Hc(e, {quoteProps:"dom", warn(f) {
    console.warn(X(f, "yellow"));
    console.warn(X(" in %s", "grey"), a);
  }});
  e = e.replace(/^import (['"])(.+?\.css)\1/gm, (f, g, k) => {
    try {
      var l = v(c, "css-injector.js");
      wa(l) || B(l, `export default ${Ic.toString()}`);
      const h = v(t(a), k);
      k = `${k}.js`;
      const m = v(c, k);
      let n = w(t(m), l);
      ...
      l += `__$styleInject(\`${p}\`)`;
      B(m, l);
      console.error("Added %s in %s", X(k, "yellow"), a);
      return `import ${g}${k}${g}`;
    } catch (h) {
      return console.error("Could not include CSS in %s:\n%s", a, X(h.message, "red")), f;
    }
  });
  return b ? `${d ? "const { h } = requir" + `e('${b}');` : `import { h } from '${b}'`}
${e}` : e;
};
const Kc = fb("alamode"), Lc = (a, b) => a.includes(b) || a.some(c => b.startsWith(`${c}/`)), Y = async a => {
  const {input:b, l:c = ".", name:d, B:e = "-", noSourceMaps:f, extensions:g, debug:k, ba:l = d} = a;
  var h = v(c, d);
  a = "-" == e;
  ...
  await G(a);
  if (!Mc(h, g)) {
    return await db({source:m, destination:a}), a;
  }
  const p = await oc({source:m, destination:a, debug:k, noSourceMaps:f});
  if ("-" != e) {
    var q = A(m);
    ({mode:q} = q);
    va(a, q);
    ...
    Db({destination:a, file:h, name:d, aa:n, source:m, u:p});
  } else {
    f || k || (h = Cb({file:h, u:p, N:w(n || "", m)}), h = "/" + `/# sourceMappingURL=data:application/json;charset=utf-8;base64,${Buffer.from(h).toString("base64")}`, console.log("\n\n%s", h));
  }
  return a;
}, Oc = async a => {
  const {input:b, B:c, l:d = ".", L:e, O:f} = a, g = v(b, d), k = v(c, d), {content:l} = await J(g);
  await Object.keys(l).reduce(async(h, m) => {
    await h;
    h = v(g, m);
    var {type:n} = l[m];
    "File" == n ? Lc(a.ignore, v(d, m)) || (e && /jsx$/.test(m) ? (a.j && (n = m.replace(/jsx$/, "js"), h = await Y({...a, name:m, ba:n}), m = n), n = v(k, m), await Nc(h, f, c, n, {l:d, name:m, ignore:a.ignore, j:a.j})) : e && a.j ? await Y({...a, name:m}) : e ? await Sa(h, k) : await Y({...a, name:m})) : "Directory" == n && (m = v(d, m), await Oc({...a, l:m}));
  }, {});
}, Nc = async(a, b, c, d, {j:e, l:f, name:g, ignore:k} = {}) => {
  if (k && (f = v(f, g), Lc(k, f))) {
    return;
  }
  a = await Jc(a, b, c, e);
  ...
  await G(b);
  await Za(b, a);
}, Mc = (a, b) => b.some(c => a.endsWith(c)), Pc = async a => {
  ...
  if (f.isDirectory()) {
    ...
    if (f.isFile()) {
      if (f = sa(b), d && /jsx$/.test(f)) {
        if (a = "-" == c ? "-" : v(c, f), f = await Nc(b, e, c, a), "-" == a) {
          return console.log(f);
        }
      } else {
        await Y({...a, input:t(b), l:"./", name:f});
      }
    }
  }
...
const Qc = async a => {
  var b = await I(a);
  const c = new V([{re:/^ *(?:var|let|const)\s+(\S+?)(\s*)=(\s*)require\((['"])(.+?)\4\)/gm, replacement(d, e, f, g, k, l) {
...
async function Rc(a) {
  ...
  var b = A(a);
  b.isDirectory() ? ({content:b} = await J(a), a = cb(b, a), await Promise.all(a.map(async c => {
    await Qc(c);
  }))) : b.isFile() && await Qc(a);
}
;function Sc() {
  const {usage:a = {}, description:b, line:c, example:d} = {usage:Tc, description:"A tool to transpile JavaScript packages using regular expressions.\nSupports import/export and JSX transpilation.\nhttps://artdecocode.com/alamode/", line:"alamode source [-o destination] [-i dir,file] [-s] [-jp]", example:"alamode src -o build"};
  var e = Object.keys(a);
  const f = Object.values(a), [g] = e.reduce(([h = 0, m = 0], n) => {
    const p = a[n].split("\n").reduce((q, u) => u.length > q ? u.length : q, 0);
    p > m && (m = p);
...
(async() => {
  try {
    ...
    if (ra) {
      return await Rc({input:ea, ignore:a, extensions:b});
    }
    let c = !1;
    oa ? c = "preact" : pa && (c = "@externs/preact");
    await Pc({input:ea, B:fa, noSourceMaps:ka, ignore:a, extensions:b, O:c, L:ma, debug:qa, j:na});
  } catch (a) {
